...
class DispatchAbstract implements DispatchInterface
{
    ...
    public function postCodRtcDispatchTransfer($request)
    {
        ...
        $user_name  = Auth::select('NAME')->where('PK_NO',$request["user_id"])->first();
            ...
            if ($request["is_shelve"] == 1) {
                Stock::whereIn('PK_NO',$inv_pks)
                    ...
                    ->update([
                        ...
                        ,'ZONE_CHECK_OUT_BY_NAME'=> $user_name->NAME
                    ]);
                    // ->get();
            }else{
                Stock::whereIn('PK_NO',$inv_pks)
                    ->update([
                        ...
                        ,'ZONE_CHECK_IN_BY_NAME'=> $user_name->NAME
                    ]);
            }
            // echo '<pre>';
            // echo '======================<br>';
    ...
    public function postDispatch($request)
    {
        ...
        try {
            if (count($data) == count($request['data'])) {
                ...
                $username = Auth::select('NAME')->where('PK_NO',$request['user_id'])->first();

                if ($dispatch_details > 0) {
                    Dispatch::where('PK_NO',$dispatch_pk->F_SC_ORDER_DISPATCH_NO)->whereRaw('(IS_DISPATHED = 0 OR IS_DISPATHED = 2)')->update(['IS_DISPATHED'=>2,'DISPATCH_USER_NAME'=>$username->NAME,'F_DISPATCH_BY_USER_NO'=>$request['user_id']]);
                }else{
                    Dispatch::where('PK_NO',$dispatch_pk->F_SC_ORDER_DISPATCH_NO)->whereRaw('(IS_DISPATHED = 0 OR IS_DISPATHED = 2)')->update(['IS_DISPATHED'=>1,'DISPATCH_USER_NAME'=>$username->NAME,'F_DISPATCH_BY_USER_NO'=>$request['user_id']]);
                }
            ...
            $order = Order::select('F_BOOKING_NO','IS_SELF_PICKUP','IS_RESELLER','F_CUSTOMER_NO','F_RESELLER_NO','DELIVERY_EMAIL')
    ...
    public function postCodRtcDispatch($request)
    {
        ...
        try {
            ...
            $dispatch->DISPATCH_USER_NAME   = $username->NAME;
            ...
            $dispatch->COLLECTED_BY         = $request['collected_by'] ?? $order->CUSTOMER_NAME ?? $order->RESHOP_NAME ?? '';
            ...
            if($booking_details_no){
                ...
                foreach ($booking_details_no as $key => $value) {
                    ...
                    if (!empty($shelve)) {
                        Stock::where('PK_NO',$shelve->inv_pk)->update(['F_INV_ZONE_NO'=>null,'INV_ZONE_BARCODE'=>null,'ZONE_CHECK_OUT_BY'=>$request['user_id'],'ZONE_CHECK_OUT_BY_NAME'=>$username->NAME]);
