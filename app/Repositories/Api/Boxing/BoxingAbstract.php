...
class BoxingAbstract implements BoxingInterface
{
    ...
    public function getProductBox($request)
    {
        ...
        $box_label_type = substr($data['box_label'],0,1);
        $box_label_type = (int)$box_label_type;

        if ($data['merchant_id'] > 0 && ($box_label_type < 3 || $box_label_type > 4)) {
            return $this->successResponse(200, 'Please provide merchant boxs !', $response_data, $response_status);
        }
        if ($data['merchant_id'] == 0 && ($box_label_type < 1 || $box_label_type > 2)) {
            return $this->successResponse(200, 'Please provide azuramart box !', $response_data, $response_status);
        }
        ...
        try {
            ...
            if ($is_update == 0) {
                if ($data['merchant_id'] == 0) {
                    ...
                    DB::statement('CALL PROC_SC_BOX_INV_STOCK_MER(:box_label, :string, :count, :col_parameters, :col_separatior, :row_separatior, :user_id, :is_update, :width, :length, :height, :weight, :merchand_id, @OUT_STATUS);',
                        array(
                            $box_label
                            ,$string
                            ,$count
                ...
                $box = Box::select('F_MERCHANT_NO')->where('BOX_NO',$box_label)->first();
                if ($data['merchant_id'] != $box->F_MERCHANT_NO) {
                    return $this->successResponse(200, 'Box and merchant mismatched !', $response_data, $response_status);
                }
                if ($data['merchant_id'] == 0) {
                    ...
                    DB::statement('CALL PROC_SC_BOX_INV_STOCK_ITEM_ADD_MER(:box_label, :string, :count, :col_parameters, :col_separatior, :row_separatior, :user_id, :is_update, :width, :length, :height, :weight, :merchand_id, @OUT_STATUS);',
                        array(
                            $box_label
                            ,$string
                            ,$count
                ...
                if ($data['merchant_id'] == 0) {
                    ...
                    DB::statement('CALL PROC_SC_BOX_INV_STOCK_ITEM_REMOVE_MER(:box_label, :string, :count, :col_parameters, :col_separatior, :row_separatior, :user_id, :is_update, :width, :length, :height, :weight, :merchand_id, @OUT_STATUS);',
                        array(
                            $box_label
                            ,$string
                            ,$count
    ...
    {
        $box = DB::table('SC_BOX')->select('PK_NO','F_INV_WAREHOUSE_NO','BOX_STATUS','WIDTH_CM as width','LENGTH_CM as length','HEIGHT_CM as height','WEIGHT_KG as weight','F_MERCHANT_NO')->where('BOX_NO',$request->box_label)->first();
        $user_map = DB::table('SS_INV_USER_MAP')->select('F_INV_WAREHOUSE_NO')->where('F_USER_NO', $request->user_id)->first();
        if(($box->F_MERCHANT_NO != $request->merchant_id)){
            return $this->successResponse(200, 'Box and merchant mismatched !', null, 0);
        }
        if (empty($box)) {
        ...
        if ($request->merchant_id == 0) {
            ...
            $get_ig = Stock::selectRaw('(SELECT IFNULL(COUNT(IG_CODE),0) from MER_INV_STOCK where IG_CODE = mkt_id and F_INV_WAREHOUSE_NO = '.$box->F_INV_WAREHOUSE_NO.' and (F_BOX_NO IS NULL OR F_BOX_NO = 0))')->limit(1)->getQuery();
                ...
                if(!empty($booking_details_ids)){
                    ...
                    if(!empty($booking_details)){
                        foreach ($booking_details as $key => $value) {
                            if (isset($value->booking->getOrder)) {
    ...
    public function postBoxList($request)
    {
        ...
        // ->select('b.PK_NO','b.CODE as box_no', 'b.BOX_NO as box_label','b.ITEM_COUNT as product_count','b.USER_NAME as user_name','b.BOX_STATUS as status','sb.F_SHIPMENT_NO as shipment_no_P','sb.BOX_SERIAL as box_serial','sb.F_BOX_NO as box_no_pk','b.F_INV_WAREHOUSE_NO'
        ...
        if($request->shipment_no && $request->shipment_no != 0) {
            $data = DB::SELECT("SELECT b.PK_NO, b.CODE as box_no, b.BOX_NO as box_label, b.ITEM_COUNT as product_count, b.USER_NAME as user_name, b.BOX_STATUS as status, sb.F_SHIPMENT_NO as shipment_no, sb.BOX_SERIAL as box_serial, sb.F_BOX_NO as box_no_pk, b.F_INV_WAREHOUSE_NO ,NULL AS MERCHANT_NAME , (select NAME from INV_WAREHOUSE where INV_WAREHOUSE.PK_NO = b.F_INV_WAREHOUSE_NO) as warehouse
            , (select CODE from SC_SHIPMENT where shipment_no=PK_NO) as shipment_label
           -- , (select IFNULL(COUNT(PK_NO),0) from INV_STOCK where PRODUCT_STATUS = 60 and F_BOX_NO = box_no_pk) as unboxed_qty
            from SC_BOX as b left join SC_SHIPMENT_BOX as sb on sb.F_BOX_NO = b.PK_NO where sb.F_SHIPMENT_NO = $request->shipment_no and b.F_MERCHANT_NO = 0 order by box_label desc");
        }else{
            $data = DB::SELECT("SELECT b.PK_NO, b.CODE as box_no, b.BOX_NO as box_label, b.ITEM_COUNT as product_count, b.USER_NAME as user_name, b.BOX_STATUS as status, sb.F_SHIPMENT_NO as shipment_no, sb.BOX_SERIAL as box_serial, sb.F_BOX_NO as box_no_pk, b.F_INV_WAREHOUSE_NO, NULL AS MERCHANT_NAME
            , (select NAME from INV_WAREHOUSE where INV_WAREHOUSE.PK_NO = b.F_INV_WAREHOUSE_NO) as warehouse
            , (select CODE from SC_SHIPMENT where shipment_no=PK_NO) as shipment_label
           -- , (select IFNULL(COUNT(PK_NO),0) from INV_STOCK where PRODUCT_STATUS = 60 and F_BOX_NO = box_no_pk) as unboxed_qty
            from SC_BOX as b
            left join SC_SHIPMENT_BOX as sb on sb.F_BOX_NO = b.PK_NO
            where b.BOX_STATUS < 50 and b.F_MERCHANT_NO = 0 order by box_label desc");
        }

        $unboxed_arr = array();
        $unboxed = DB::SELECT("SELECT F_BOX_NO, IFNULL(COUNT(1),0) as TOTAL FROM INV_STOCK WHERE PRODUCT_STATUS = 60 GROUP BY F_BOX_NO ");
        foreach ($unboxed as $key => $value) {
            if($value->F_BOX_NO){
                $unboxed_arr[$value->F_BOX_NO] = $value->TOTAL;
            }
        }

        if($data){
            foreach($data as $list){
                $list->unboxed_qty = $unboxed_arr[$list->box_no_pk] ?? 0;
            }
        }

        $max_air = DB::table('SC_BOX')->where('BOX_NO','<',20000000)->max('BOX_NO');
        $max_sea = DB::table('SC_BOX')->where('BOX_NO','<',30000000)->max('BOX_NO');
    ...
    public function postMerchantBoxList($request)
    {
        $user_map = DB::table('SS_INV_USER_MAP')->select('F_INV_WAREHOUSE_NO')->where('F_USER_NO', $request->user_id)->first();
        if(empty($user_map)){
            return $this->successResponse(200, 'Box Not Found !', null, 0);
        }
        if ($request->merchant_id == -1) {
            if($request->shipment_no && $request->shipment_no != 0) {
                $data = DB::SELECT("SELECT b.PK_NO, b.CODE as box_no, b.BOX_NO as box_label, b.ITEM_COUNT as product_count, b.USER_NAME as user_name, b.BOX_STATUS as status, sb.F_SHIPMENT_NO as shipment_no, sb.BOX_SERIAL as box_serial, sb.F_BOX_NO as box_no_pk, b.F_INV_WAREHOUSE_NO, (select NAME from INV_WAREHOUSE where INV_WAREHOUSE.PK_NO = b.F_INV_WAREHOUSE_NO) as warehouse
                , (select CODE from SC_SHIPMENT where shipment_no=PK_NO) as shipment_label
                ,(select NAME from SLS_MERCHANT where PK_NO = b.F_MERCHANT_NO) as MERCHANT_NAME
                from SC_BOX as b left join SC_SHIPMENT_BOX as sb on sb.F_BOX_NO = b.PK_NO where sb.F_SHIPMENT_NO = $request->shipment_no and b.F_MERCHANT_NO > 0 order by box_label desc");
            }else{
                $data = DB::SELECT("SELECT b.PK_NO, b.CODE as box_no, b.BOX_NO as box_label, b.ITEM_COUNT as product_count, b.USER_NAME as user_name, b.BOX_STATUS as status, sb.F_SHIPMENT_NO as shipment_no, sb.BOX_SERIAL as box_serial, sb.F_BOX_NO as box_no_pk, b.F_INV_WAREHOUSE_NO
                , (select NAME from INV_WAREHOUSE where INV_WAREHOUSE.PK_NO = b.F_INV_WAREHOUSE_NO) as warehouse
                , (select CODE from SC_SHIPMENT where shipment_no=PK_NO) as shipment_label
                ,(select NAME from SLS_MERCHANT where PK_NO = b.F_MERCHANT_NO) as MERCHANT_NAME
                from SC_BOX as b
            ...
            $unboxed_arr = array();
            $unboxed = DB::SELECT("SELECT F_BOX_NO, IFNULL(COUNT(1),0) as TOTAL FROM MER_INV_STOCK WHERE PRODUCT_STATUS = 60 GROUP BY F_BOX_NO ");
        }else{
            if($request->shipment_no && $request->shipment_no != 0) {
                $data = DB::SELECT("SELECT b.PK_NO, b.CODE as box_no, b.BOX_NO as box_label, b.ITEM_COUNT as product_count, b.USER_NAME as user_name, b.BOX_STATUS as status, sb.F_SHIPMENT_NO as shipment_no, sb.BOX_SERIAL as box_serial, sb.F_BOX_NO as box_no_pk, b.F_INV_WAREHOUSE_NO, (select NAME from INV_WAREHOUSE where INV_WAREHOUSE.PK_NO = b.F_INV_WAREHOUSE_NO) as warehouse
                ,(select CODE from SC_SHIPMENT where shipment_no=PK_NO) as shipment_label
                ,(select NAME from SLS_MERCHANT where PK_NO = b.F_MERCHANT_NO) as MERCHANT_NAME
                from SC_BOX as b left join SC_SHIPMENT_BOX as sb on sb.F_BOX_NO = b.PK_NO where sb.F_SHIPMENT_NO = $request->shipment_no and b.F_MERCHANT_NO = $request->merchant_id order by box_label desc");
            }else{
                $data = DB::SELECT("SELECT b.PK_NO, b.CODE as box_no, b.BOX_NO as box_label, b.ITEM_COUNT as product_count, b.USER_NAME as user_name, b.BOX_STATUS as status, sb.F_SHIPMENT_NO as shipment_no, sb.BOX_SERIAL as box_serial, sb.F_BOX_NO as box_no_pk, b.F_INV_WAREHOUSE_NO
                , (select NAME from INV_WAREHOUSE where INV_WAREHOUSE.PK_NO = b.F_INV_WAREHOUSE_NO) as warehouse
                , (select CODE from SC_SHIPMENT where shipment_no=PK_NO) as shipment_label
                ...
                from SC_BOX as b
                left join SC_SHIPMENT_BOX as sb on sb.F_BOX_NO = b.PK_NO
                where b.BOX_STATUS < 50 and b.F_MERCHANT_NO = $request->merchant_id order by box_label desc");
            }
            $unboxed_arr = array();
            $unboxed = DB::SELECT("SELECT F_BOX_NO, IFNULL(COUNT(1),0) as TOTAL FROM MER_INV_STOCK WHERE PRODUCT_STATUS = 60 AND F_MERCHANT_NO = $request->merchant_id GROUP BY F_BOX_NO ");
        }
        foreach ($unboxed as $key => $value) {
            if($value->F_BOX_NO){
                $unboxed_arr[$value->F_BOX_NO] = $value->TOTAL;
        ...
        $max_air = DB::table('SC_BOX')->where('BOX_NO','<',40000000)->max('BOX_NO');
        $max_sea = DB::table('SC_BOX')->where('BOX_NO','<',50000000)->max('BOX_NO');
    ...
    {
        $box_no = DB::table('SC_BOX')->select('PK_NO','F_INV_WAREHOUSE_NO','F_MERCHANT_NO')->where('PK_NO',$request->PK_NO)->first();
        $user_map = DB::table('SS_INV_USER_MAP')->select('F_INV_WAREHOUSE_NO')->where('F_USER_NO', $request->user_id)->first();
        if (empty($box_no)) {
            return $this->successResponse(200, 'Box not found !', null, 0);
        }
        ...
        if ($box_no->F_MERCHANT_NO == 0) {
            ...
            $get_ig = MerStock::selectRaw('(SELECT IFNULL(COUNT(IG_CODE),0) from MER_INV_STOCK where IG_CODE = mkt_id and F_INV_WAREHOUSE_NO = '.$box_no->F_INV_WAREHOUSE_NO.' and (F_BOX_NO IS NULL OR F_BOX_NO = 0))')->limit(1)->getQuery();
    ...
    public function postBoxLabelExists($request)
    {
        ...
        $box = Box::select('PK_NO','F_INV_WAREHOUSE_NO','BOX_STATUS','WIDTH_CM as width','LENGTH_CM as length','HEIGHT_CM as height','WEIGHT_KG as weight','F_MERCHANT_NO')->where('BOX_NO',$request->box_label)->first();
        if (empty($box)) {
            return $this->successResponse(200, 'Box not found !', null, 0);
    ...
    {
        $data = DB::table('SLS_MERCHANT')
                ->select('PK_NO AS merchant_pk','NAME AS merchant_name')
                ->where('IS_ACTIVE',1)
                ->get();
        if ($data->count()>0) {
            return $this->successResponse(200, 'Data found !', $data, 1);
