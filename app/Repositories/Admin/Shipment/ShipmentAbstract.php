...
class ShipmentAbstract implements ShipmentInterface
{
        ...
        if ($request->is_update == 0) {
            ...
            try {
               ...
               // $shipment->F_SENT_BY              = $request->delivery_address;
                ...
                if($request->shipment_for == 2){
                    $shipment->SHIPMENT_FOR = 2;
                    $shipment->F_MERCHANT_NO = $request->merchant_no;
                }else{
                    $shipment->SHIPMENT_FOR = 1;
                }
    ...
    public function addShipmentBox($request)
    {
        ...
        $shipment_master = Shipment::select('PK_NO','F_FROM_INV_WAREHOUSE_NO','CODE','SHIPMENT_STATUS','IS_AIR_SHIPMENT','F_MERCHANT_NO')->where('CODE', $shipment_label)->whereRaw('( SHIPMENT_STATUS IS NULL OR SHIPMENT_STATUS = 10 )')->first();
        ...
        $box_master = Box::select('PK_NO','CODE','BOX_NO','ITEM_COUNT','F_MERCHANT_NO')->where('BOX_NO', $box_label)->whereRaw('BOX_STATUS = 10')->first();
        if (empty($box_master)) {
            return $response = $flag+2;
        }
        if ($shipment_master->SHIPMENT_FOR == 1) {//AZURAMART SHIPMENT
            if ($box_master->F_MERCHANT_NO > 0) {
                return $response = 400;
            }
        }else{//MERCAHNT SHIPMENT
        ...
        if ($shipment_master->SHIPMENT_FOR == 1) {
            $inv_stock = Stock::select('PREFERRED_SHIPPING_METHOD','FINAL_PREFFERED_SHIPPING_METHOD')->where('F_BOX_NO', $box_master->PK_NO)->get();
        }else{
            $inv_stock = MerStock::select('PREFERRED_SHIPPING_METHOD','FINAL_PREFFERED_SHIPPING_METHOD')->where('F_BOX_NO', $box_master->PK_NO)->get();
        }
        ...
        try {
            ...
            if ($request->status == 20) {
                    ...
                    if($request->invoice_for == 'merchant'){
                        MerStock::where('F_SHIPPMENT_NO', $request->shipment_id)->whereIn('F_BOX_NO', $box_id)->update(['PRODUCT_STATUS' => 30]);
                    }
            }
            if ($request->status == 30) {
                $invoice_for = $request->invoice_for;
                DB::statement('CALL PROC_SC_SHIPMENT_CANCELLED(:IN_INVOICE_FOR,:IN_SHIPMENT_PK_NO, @OUT_STATUS);',array($invoice_for,$request->shipment_id));
            ...
            if ($request->status == 40) {
                ...
                if($request->invoice_for == 'merchant'){
                    MerStock::where('F_SHIPPMENT_NO', $request->shipment_id)->update(['PRODUCT_STATUS' => 40]);
                }
            }
            if ($request->status == 70) {
                    ...
                    if($request->invoice_for == 'merchant'){
                        MerStock::where('F_SHIPPMENT_NO', $request->shipment_id)->update(['PRODUCT_STATUS' => 45]);
                    }
            }
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function postShipmentPackaging($request, $id, $is_update)
    {
        ...
        try {
            if($request->invoice_for == 'azuramart'){
                DB::statement('CALL PROC_SC_PACKAGING_LIST_INV_STOCK(:shipment_no, :is_update, @OUT_STATUS);',array($id,$is_update));
            }

            if($request->invoice_for == 'merchant'){
                DB::statement('CALL PROC_MER_SC_PACKAGING_LIST_INV_STOCK(:shipment_no, :is_update, @OUT_STATUS);',array($id,$is_update));
            }
    ...
    public function getShipment($id)
    {
        if (Auth::user()->F_MERCHANT_NO > 0) {
            $data['data'] = Shipment::where('F_MERCHANT_NO',Auth::user()->F_MERCHANT_NO)->find($id);
        }else{
        ...
        $data['box'] = Box::join('SC_SHIPMENT_BOX','SC_SHIPMENT_BOX.F_BOX_NO','SC_BOX.PK_NO')
                        ...
                        if (Auth::user()->F_MERCHANT_NO > 0) {
                            $data['box'] = $data['box']->where('SC_BOX.F_MERCHANT_NO',Auth::user()->F_MERCHANT_NO);
                        }
                        $data['box'] = $data['box']->orderBy('SC_SHIPMENT_BOX.BOX_SERIAL')
                        ->get();
        if (!$data['box']->isEmpty()) {
