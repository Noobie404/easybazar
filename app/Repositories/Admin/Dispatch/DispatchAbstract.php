...
class DispatchAbstract implements DispatchInterface
{
    ...
    public function __construct(Agent $agent,Booking $booking, Order $order,Country $country, Dispatch $dispatch)
    {
        $this->agent   = $agent;
        ...
        $query = $this->dispatch->select('SC_ORDER_DISPATCH.*','o.DISPATCH_STATUS','bd.IS_COLLECTED_FOR_RTS')
                ...
                if($request->keywords != ''){
                    ...
                    if($pieces){
                        ...
                        foreach ($pieces as $key => $piece) {
                            $query->orWhere('o.RESHOP_NAME', 'LIKE', '%' . $piece . '%');
                        }
                        foreach ($pieces as $key => $piece) {
    ...
    public function postStore($request)
    {
        ...
        if($request->dispatch_type == 'rts'){
            $first_cnsignment = OrderConsignment::where('F_ORDER_NO', $order->PK_NO)->orderBy('PK_NO', 'DESC')->first();
        ...
        $dispatch->DISPATCH_USER_NAME    = Auth::user()->NAME;
        ...
        if($request->booking_details_no){
            ...
            foreach ($request->booking_details_no as $key => $value) {
                ...
                if($request->dispatch_qty[$key] > 0 ){
                    ...
                    $shelve = Stock::select('INV_WAREHOUSE_ZONES.ITEM_COUNT','INV_WAREHOUSE_ZONES.PK_NO')->join('INV_WAREHOUSE_ZONES','INV_WAREHOUSE_ZONES.PK_NO','INV_STOCK.F_INV_ZONE_NO')->where('INV_STOCK.PK_NO',$booking_details->F_INV_STOCK_NO)->first();

                    if (!empty($shelve)) {
                        Stock::where('PK_NO',$booking_details->F_INV_STOCK_NO)->update(['INV_ZONE_BARCODE'=>null,'F_INV_ZONE_NO'=>null,'ZONE_CHECK_OUT_BY'=>Auth::user()->PK_NO,'ZONE_CHECK_OUT_BY_NAME'=>Auth::user()->NAME]);
    ...
    public function postAssignOrderItem($request)
    {
        ...
        DB::commit();
            $username =\DB::table('SA_USER')
                        ...
                        ->first();
            $user_assigned = isset($username->NAME) ? 1 : 0;
            return ['status'=>1,'name'=>$username->NAME ?? 'Unassigned','id'=>$request->logistic_user,'skuid'=>$request->sku_id,'user_assigned'=>$user_assigned];
    }
