...
class OfferAbstract implements OfferInterface
{
    ...
    public function postStore($request)
    {
        ...
        try {
            ...
            $check = Offer::where('URL_SLUG', $slug)->first();
            if($check){
                $slug = $slug.'-'.rand(1,99);
            }
            $offer = new Offer();
            ...
            if($request->file('banner_image')){
                ...
                $file_name1     = 'bundle_'. date('dmY'). '_' .uniqid(). '.' . $banner_image->getClientOriginalExtension();
                $offer->BANNER_IMAGE   = '/media/images/bundle/'.$offer->PK_NO.'/'.$file_name1;
                $banner_image->move(public_path().'/media/images/bundle/'.$offer->PK_NO.'/', $file_name1);
            }
            $offer->update();
    ...
    public function postUpdate($request, $pk_no)
    {
        ...
        try {
            $slug = Str::slug(strtolower($request->public_name));
            $check = Offer::where('URL_SLUG', $slug)->where('PK_NO','!=',$pk_no)->first();
            if($check){
                $slug = $slug.'-'.rand(1,99);
            }
            ...
            if($request->file('banner_image')){
                $banner_image   = $request->file('banner_image');
                $file_name1     = 'bundle_'. date('dmY'). '_' .uniqid(). '.' . $banner_image->getClientOriginalExtension();
                $offer->BANNER_IMAGE   = '/media/images/bundle/'.$pk_no.'/'.$file_name1;
                $banner_image->move(public_path().'/media/images/bundle/'.$pk_no.'/', $file_name1);
            }
