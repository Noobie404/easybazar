...
class BrandAbstract implements BrandInterface
{
    ...
    {
        $data = $this->brand->select('PRD_BRAND.*')->where('PRD_BRAND.IS_ACTIVE',1);
        if($request->brand != ''){
            $data->where('PRD_BRAND.NAME', 'LIKE', '%' . $request->brand . '%');
        }
          if($request->size != ''){
            $data->leftJoin('PRD_SIZE','PRD_SIZE.F_BRAND_NO','PRD_BRAND.PK_NO');
            $data->where('PRD_SIZE.NAME', 'LIKE', '%' . $request->size . '%');
        }
        if($request->color != ''){
            $data->leftJoin('PRD_COLOR','PRD_COLOR.F_BRAND','PRD_BRAND.PK_NO');
            $data->where('PRD_COLOR.NAME', 'LIKE', '%' . $request->color . '%');
        }

        $data = $data->orderBy('PRD_BRAND.NAME', 'ASC')->paginate(10);
    ...
    public function postStore($request)
    {
        ...
        try {
            ...
            {
                $brand->BANNER      = $this->uploadImage($request->banner);
            }
