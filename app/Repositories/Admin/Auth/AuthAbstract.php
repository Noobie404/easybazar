...
class AuthAbstract implements AuthInterface
{
    ...
    public function postStore($request)
    {
        ...
        if ($request->profile_pic != '') {
            ...
            $auth->PROFILE_PIC_URL = '/media/images/profile/'.$file_name;
    ...
    public function postUpdate($request, int $id, string $type = null)
    {
        ...
        $auth->MOBILE_NO = $request->mobile_no;
        $auth->GENDER = $request->gender;
        if ($type == 'single' && $request->password != '') {
            $auth->PASSWORD = Hash::make($request->password);
        }else{
            if($request->password){
                $auth->PASSWORD = Hash::make($request->password);
        ...
        if ($request->profile_pic != '') {
            ...
            if(File::exists(public_path('media/images/profile/'.$id.'/'.$auth->PROFILE_PIC))) {
                File::delete(public_path('media/images/profile/'.$id.'/'.$auth->PROFILE_PIC));
            }
            $file_name = 'profile_'. date('dmY'). '_' .time(). '.' . $request->profile_pic->getClientOriginalExtension();
            $request->profile_pic->move(public_path('media/images/profile/'.$id), $file_name);
            $auth->PROFILE_PIC_URL = '/media/images/profile/'.$id.'/'.$file_name;
            $auth->PROFILE_PIC = $file_name;
        }
