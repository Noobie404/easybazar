...
class MerStock extends Model
{
    protected $table = 'MER_INV_STOCK';
    ...
    public function box_list_per_ship($id,$inv)
    {
        ...
        $data = MerStock::select('F_BOX_NO',DB::raw('(select IFNULL(COUNT(F_BOX_NO),0)) as boxed'))
    ...
    public function get_box_details($shipment_no,$skuid,$warehouse=null,$invoiceid,$invoicetype)
    {
        ...
        $data = MerStock::select('F_BOX_NO as f_box_no1','BOX_BARCODE'
    ...
    {
        $data['info'] = MerStock::join('PRC_STOCK_IN','PRC_STOCK_IN.PK_NO','MER_INV_STOCK.F_PRC_STOCK_IN_NO')
                                ->select('PRC_STOCK_IN.MASTER_INVOICE_RELATIVE_PATH','PRC_STOCK_IN.INVOICE_NO','PRC_STOCK_IN.INVOICE_DATE','MER_INV_STOCK.PRODUCT_PURCHASE_PRICE')
                                ->where('PRC_STOCK_IN.PK_NO',$invoice_id)
                                ->where('MER_INV_STOCK.SKUID',$sku_id)
                                ->groupBy('PRC_STOCK_IN.PK_NO')
                                // ->orderBy('PRC_STOCK_IN.INVOICE_DATE','ASC')
                                ->first();
        $data['total'] = MerStock::where('F_PRC_STOCK_IN_NO',$invoice_id)->where('SKUID',$sku_id)->count();
        $data['dispatched'] = MerStock::where('F_PRC_STOCK_IN_NO',$invoice_id)->where('SKUID',$sku_id)->where('ORDER_STATUS','80')->count();
