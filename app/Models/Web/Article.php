...
class Article extends Model
{
    ...
    public function postStore($request)
    {
        ...
        try {
            ...
            $article->F_SS_CREATED_BY                      = Auth::user()->PK_NO;
            $article->SS_CREATED_ON                        = date("Y-m-d h:i:s", time());
            if ($request->hasFile('feature_image'))
            {
            $image              = $request->file('feature_image');
            $extension          = $image->getClientOriginalExtension();
            $feature_path       = 'media/images/blog';
            $thumb_path         = 'media/images/blog/thumb';
     ...
     public function postUpdate($request)
    {
        ...
        try {
            ...
            $article->META_KEYWARDS                        = $request->meta_keywards;
            $article->META_DESCRIPTION                     = $request->meta_description;
            $article->F_SS_MODIFIED_BY                     = Auth::user()->PK_NO;
            $article->SS_MODIFIED_ON                       = date("Y-m-d h:i:s", time());
            if ($request->hasFile('feature_image'))
            {
            if (File::exists(public_path($article->FEATURE_IMAGE))) {
                File::delete(public_path($article->FEATURE_IMAGE));
            }
            if (File::exists(public_path($article->THUMBNAIL_IMAGE))) {
                File::delete(public_path($article->THUMBNAIL_IMAGE));
            }
            $image          = $request->file('feature_image');
            $extension      = $image->getClientOriginalExtension();
            $feature_path       = 'media/images/blog';
    ...
    public function textEditorImageUpload(Request $request)
    {
        ...
        $extension = $image->getClientOriginalExtension();
        $destinationPath1 = 'media/images/blog';
        $base_name = preg_replace('/\..+$/', '', $image->getClientOriginalName());
        $base_name = explode(' ', $base_name);
        $base_name = implode('-', $base_name);
        $img = Image::make($image->getRealPath());
