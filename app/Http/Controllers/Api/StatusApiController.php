...
class StatusApiController extends Controller
{
    ...
    public function checkStatus(Request $request){
        ...
        */
        ...
        //SEND SMS
        $notis   = DB::table('SLS_NOTIFICATION')->select('PK_NO','MOBILE_NO','BODY')->where('IS_SEND',0)->get();
        if($notis){
            foreach ($notis as $key => $value) {
                ...
                if($smsRes){
                    DB::table('SLS_NOTIFICATION')->where('PK_NO',$value->PK_NO)->update(['IS_SEND' => 1, 'SEND_AT' => date('Y-m-d H:i:s')]);
