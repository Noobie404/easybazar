...
class DashboardController extends BaseController
{
    ...
    public function getOtions($table,$select_key,$select_val,$cond_col,$cond_val){
        $data = DB::table($table)->where($cond_col,$cond_val)->orderBy($select_val,'ASC')->pluck($select_val,$select_key);
        $response = false;
        $html   = '';
    ...
    public function getIndex() {
        ...
        // dd(Auth::user());
        // $product = DB::table('PRD_MASTER_SETUP')->where('IS_ACTIVE',1)
        // ->take(100)
        // ->orderBy('PK_NO','DESC')
        // ->get();
        // $shop_id = 82;
        // DB::table('PRD_SHOP_MASTER_MAP')->where('F_SHOP_NO',$shop_id)->delete();
        // foreach ($product as $key => $value) {
        //     $data = [
        //         'F_PRD_MASTER_SETUP_NO'=>$value->PK_NO,
        //         'F_SHOP_NO'=>$shop_id,
        //         'IS_ADMIN_CREATED'=>1,
        //         'F_SS_CREATED_BY'=>1,
        //         'SS_CREATED_ON'=>Carbon::now()
        // ];
        //  DB::table('PRD_SHOP_MASTER_MAP')->insert($data);
        // }
        //INSERT
        // $products = DB::table('PRD_SHOP_MASTER_MAP')->get();
        // foreach ($products as $key => $value) {
        //     DB::table('PRD_SHOP_VARIANT_MAP')
        //     ->where('F_SHOP_NO',$value->F_SHOP_NO)
        //     ->where('F_PRD_MASTER_SETUP_NO',$value->F_PRD_MASTER_SETUP_NO)
        //     ->delete();
        //     $variant = DB::table('PRD_VARIANT_SETUP')->where('F_PRD_MASTER_SETUP_NO',$value->F_PRD_MASTER_SETUP_NO)->get();
        //     foreach ($variant as $key => $val) {
        //         $data = [
        //             'F_PRD_MASTER_SETUP_NO' =>$value->F_PRD_MASTER_SETUP_NO,
        //             'F_PRD_VARIANT_NO' =>$val->PK_NO,
        //             'F_SHOP_NO'=>$value->F_SHOP_NO,
        //             'IS_ADMIN_CREATED'=>1,
        //             'F_SS_CREATED_BY'=>1,
        //             'SS_CREATED_ON'=>Carbon::now()
        //             ];
        //      DB::table('PRD_SHOP_VARIANT_MAP')->insert($data);
        //      $checkExist = DB::table('PRD_VARIANT_STOCK_QTY')->where('F_PRD_VARIANT_NO',$val->PK_NO)->first();
        //      if(empty($checkExist)){
        //         $_data = [
        //             'F_PRD_MASTER_SETUP_NO'=> $value->F_PRD_MASTER_SETUP_NO,
        //             'F_PRD_VARIANT_NO'=> $val->PK_NO,
        //             'F_SHOP_NO' => $value->F_SHOP_NO,
        //             'TOTAL_FREE_STOCK' => 0,
        //             'SS_MODIFIED_ON' => Carbon::now(),
        //             'F_SS_CREATED_BY'=>1
        //         ];
        //         DB::table('PRD_VARIANT_STOCK_QTY')->insert($_data);
        //      }
        //     }
        // }
        /*
        ...
        */
        ...
        if(Auth::user()->USER_TYPE == 10){
            return view('admin.dashboard.reseller',compact('data'));
        }else{
        ...
        if(hasAccessAbility('view_dashboard_cards_sales_agent', $roles) || hasAccessAbility('view_dashboard_cards_my_manager', $roles)){
            ...
            $cod_rtc_rts = Booking::select('SLS_BOOKING.SS_CREATED_ON','SLS_BOOKING.DISPATCH_STATUS','SLS_BOOKING.IS_ADMIN_HOLD','SLS_BOOKING.IS_SELF_PICKUP')

                            ->whereRaw('SLS_BOOKING.DISPATCH_STATUS < 40')
            ...
            foreach ($cod_rtc_rts as $key => $value) {
                if ($value->DISPATCH_STATUS >= 20 && $value->DISPATCH_STATUS <= 30 && $value->IS_SELF_PICKUP == 0 && $value->IS_ADMIN_HOLD == 0) {
                    $rts_today++;
                }

                if ($value->IS_SELF_PICKUP == 1 && $value->IS_ADMIN_HOLD == 0 ) {
                    $cod_rtc_today++;
                }
            }
        ...
        if($role_id->F_ROLE_NO == 1){
            ...
            $inv_stock = Stock::select('F_BOOKING_NO','BOOKING_STATUS','PRODUCT_PURCHASE_PRICE','REGULAR_PRICE')->whereRaw('(BOOKING_STATUS IS NULL OR BOOKING_STATUS < 80) AND (PRODUCT_STATUS IS NULL OR PRODUCT_STATUS < 420)')->get();
            ...
            $payment_verfication_pending = AccBankTxn::whereIn('IS_CUS_SELLER_BANK_RECONCILATION',[1,2])->sum('AMOUNT_ACTUAL');
            ...
            $data['payment_verfication_pending']        = $payment_verfication_pending ?? 0;
    ...
    public function getDocumentationView(Request $request){
        $row = DB::table('WEB_DOCUMENTATION')->first();
        return view('documentation.view')->withRow($row);
    }

    public function getDocumentation(Request $request){
        $row = DB::table('WEB_DOCUMENTATION')->first();
        return view('documentation.index')->withRow($row);
    }

    public function postDocumentationStore(Request $request){
        DB::beginTransaction();
        try {
            if(!empty($request->id)){
               $data = DB::table('WEB_DOCUMENTATION')->where('PK_NO',$request->id)->update(['DESCRIPTION'=>$request->description]);
            }
            else{
             $data = DB::table('WEB_DOCUMENTATION')->insert(['DESCRIPTION'=>$request->description]);
            }
         } catch (\Exception $e) {
            dd($e->getMessage());
            DB::rollback();
            return $this->successResponse(200, 'Unable to update',null,0);
        }
        DB::commit();

        return $this->successResponse(200,'Successfully updated',$data,1);
    }
    ...
    public function textEditorImageUpload(Request $request)
    {
        if (!is_null($request->file('image')))
        {
            ...
            $base_name = Str::slug(strtolower($base_name));
            $image_name = $base_name . "-" . uniqid() . "." . 'webp';
            $file_path = 'media/images/photos';
            if (!file_exists($file_path)) {
                mkdir($file_path, 0755, true);
            }
            $img = Image::make($photo->getRealPath());
            $file_unique = $base_name. '-' .uniqid();
            $file_name1 = $file_unique. '.webp';
            Image::make($img)->save($file_path.'/'.$file_name1);
           return  url('/').'/'.$file_path.'/'.$file_name1;
        }
    }
}
