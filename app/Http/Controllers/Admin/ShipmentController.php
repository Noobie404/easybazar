...
class ShipmentController extends BaseController
{
    ...
    {
        $data = $this->shipmodel->select('SC_SHIPMENT.*','SLS_MERCHANT.SHORT_NAME as MERCHANT_NAME')->leftJoin('SLS_MERCHANT', 'SLS_MERCHANT.PK_NO', 'SC_SHIPMENT.F_MERCHANT_NO');
        if (Auth::user()->F_MERCHANT_NO > 0) {
            $data = $data->where('F_MERCHANT_NO',Auth::user()->F_MERCHANT_NO);
        }
        $data = $data->orderBy('SC_SHIPMENT.PK_NO','DESC')->get();
    ...
    {
        $data = Shipment::select('SC_SHIPMENT.*', 'SLS_MERCHANT.SHORT_NAME as MECHANT_NAME')->leftJoin('SLS_MERCHANT','SLS_MERCHANT.PK_NO','SC_SHIPMENT.F_MERCHANT_NO')->orderBy('PK_NO','DESC')->get();
    ...
    public function getCreate($id = null) {
        ...
        $merchants          = Merchant::where('IS_ACTIVE',1)->pluck('NAME','PK_NO');
    ...
    public function postShipmentPackaging(Request $request, $id, $is_update)
    {

        $this->resp = $this->shipment->postShipmentPackaging($request, $id,$is_update);
        return redirect()->route($this->resp->redirect_to)->with($this->resp->redirect_class, $this->resp->msg);
    }
