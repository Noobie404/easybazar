...
class PaymentController extends BaseController
{
    ...
    {
        $this->resp             = $this->paymentInt->getPaginatedList($request, 50);
    ...
    public function getCreate($customer_id = null, $type = null )
    {
        ...
        if($type == 'customer'){
            ...
            $remaining_balance = PaymentCustomer::select('PK_NO', 'F_PAYMENT_CURRENCY_NO','PAYMENT_DATE','SLIP_NUMBER','PAID_BY','PAYMENT_NOTE','PAYMENT_REMAINING_MR','PAYMENT_REMAINING_MR','IS_TRANSFERRED','F_BANK_TXN_NO_TRANSFERAR')
            ->where('F_CUSTOMER_NO',$customer_id)
                                        ->where('PAYMENT_REMAINING_MR', '>', 0)
                                        // ->where('PAYMENT_CONFIRMED_STATUS', 1)
                                        ->get();
            if($remaining_balance && $remaining_balance->count() > 0 ){
                foreach ($remaining_balance as $key => $value) {
                    if($value->IS_TRANSFERRED == 1){
                        $value->TXN_CODE = DB::table('ACC_BANK_TXN')->where('PK_NO',$value->F_BANK_TXN_NO_TRANSFERAR)->first()->CODE;
                    }else{
                        $value->TXN_CODE = DB::table('ACC_BANK_TXN')->where('F_CUSTOMER_PAYMENT_NO',$value->PK_NO)->first()->CODE;
                    }

                }
            }
            $data['remaining_balance'] = $remaining_balance;
            $data['due_bills'] = OnlinePayment::where('F_CUSTOMER_NO',$customer_id)->where('IS_PAID',0)->orderBy('PK_NO','DESC')->get();
        }
        ...
        if($type == 'reseller'){
            ...
            $remaining_balance = PaymentReseller::select('PK_NO', 'F_PAYMENT_CURRENCY_NO','PAYMENT_DATE','SLIP_NUMBER','PAID_BY','PAYMENT_NOTE','PAYMENT_REMAINING_MR','PAYMENT_REMAINING_MR','IS_TRANSFERRED','F_BANK_TXN_NO_TRANSFERAR')->where('F_RESELLER_NO',$customer_id)
                                        ->where('PAYMENT_REMAINING_MR', '>', 0)
                                        // ->where('PAYMENT_CONFIRMED_STATUS', 1)
                                        ->get();
            ...
            if($remaining_balance && $remaining_balance->count() > 0 ){
                foreach ($remaining_balance as $key => $value) {
                    if($value->IS_TRANSFERRED == 1){
                        ...
                        $value->TXN_CODE = DB::table('ACC_BANK_TXN')->where('F_RESELLER_PAYMENT_NO',$value->PK_NO)->first()->CODE;
                    }

                }
            }
            $data['remaining_balance'] = $remaining_balance;
            $data['due_bills'] = OnlinePayment::where('F_RESELLER_NO',$customer_id)->where('IS_PAID',0)->orderBy('PK_NO','DESC')->get();
        }
