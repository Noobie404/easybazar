...
class DispatchController extends BaseController
{
    ...
    public function __construct(BookingInterface $bookingInt, Booking $booking_model, Customer $customer, ProductVariant $prd_variant, Agent $agent, OrderInterface $order, DispatchInterface $dispatch)
    {
    ...
    public function getDispatch(Request $request,$id)
    {
        ...
        // }
        // $data['pos_laju '] = ApiList::find(1);
        $data['SC_ORDER_DISPATCH']  = DB::table('SC_ORDER_DISPATCH')->select('IS_DISPATHED')->where('F_ORDER_NO',$id)->first();
        $data['return_request'] = DispatchItemReturn::where('F_BOOKING_NO',$data['booking']->PK_NO)->where('STATUS',0)->get();
    ...
    public function getCollectionListBreakdown($id)
    {
        ...
        , b.TOTAL_PRICE AS ORDER_PRICE, b.DISCOUNT AS ORDER_DISCOUNT, b.BOOKING_STATUS,NULL AS XFER_PK, t.CODE AS PAYMENT_NO, u.NAME AS ENTRY_BY_NAME, cp.SS_CREATED_ON AS DATE_AT, t.PK_NO AS TX_PK_NO, NULL AS PAYMENT_VERIFY, t.AMOUNT_ACTUAL AS PAY_AMOUNT, NULL AS IS_IN, t.MATCHED_ON, 'Customer' AS TYPE, NULL AS PAYMENT_METHOD, cp.PAID_BY, NULL AS PARTY_HEAD, cp.CUSTOMER_NAME, cp.F_CUSTOMER_NO AS CUS_PK, NULL AS BANK_NAME, NULL AS BANK_ACC_NAME
        ...
        SELECT o.PK_NO AS ORDER_PK_NO, o.ORDER_ACTUAL_TOPUP, o.ORDER_BUFFER_TOPUP, o.ORDER_BALANCE_RETURN, o.DISPATCH_STATUS, b.PK_NO AS BOOKING_PK_NO, b.BOOKING_NO
        , b.TOTAL_PRICE AS ORDER_PRICE, b.DISCOUNT AS ORDER_DISCOUNT, b.BOOKING_STATUS,NULL as XFER_PK, t.CODE AS PAYMENT_NO, u.NAME AS ENTRY_BY_NAME, rp.SS_CREATED_ON AS DATE_AT, t.PK_NO AS TX_PK_NO, NULL AS PAYMENT_VERIFY, t.AMOUNT_ACTUAL AS PAY_AMOUNT, NULL as IS_IN, t.MATCHED_ON, 'Reseller' AS TYPE, NULL as PAYMENT_METHOD, rp.PAID_BY, NULL as PARTY_HEAD, rp.RESHOP_NAME as CUSTOMER_NAME, rp.F_RESELLER_NO as CUS_PK, NULL as BANK_NAME, NULL as BANK_ACC_NAME
        FROM ACC_BANK_TXN AS t
        ...
        WHERE t.F_ACC_PAYMENT_BANK_NO = $id
        AND t.IS_COD = 1
        -- SELECT NULL as ORDER_PK_NO, NULL AS ORDER_ACTUAL_TOPUP, NULL AS ORDER_BUFFER_TOPUP, NULL AS ORDER_BALANCE_RETURN, NULL AS DISPATCH_STATUS, NULL AS BOOKING_PK_NO, NULL AS BOOKING_NO, NULL AS ORDER_PRICE, NULL AS ORDER_DISCOUNT, NULL AS BOOKING_STATUS, cp.PAYMENT_DATE as DATE_AT, t.CODE AS PAYMENT_NO, cp.MR_AMOUNT AS PAY_AMOUNT, t.PK_NO as TX_PK_NO, t.IS_MATCHED AS PAYMENT_VERIFY, u.NAME as ENTRY_BY_NAME, t.MATCHED_ON, 'Payment' AS TYPE FROM ACC_CUSTOMER_PAYMENTS AS cp LEFT JOIN SA_USER as u ON u.PK_NO = cp.F_SS_CREATED_BY LEFT JOIN ACC_BANK_TXN AS t ON t.F_CUSTOMER_PAYMENT_NO = cp.PK_NO  WHERE cp.F_PAYMENT_ACC_NO = $id
    ...
    public function getConfirmList(Request $request)
    {
        ...
        return view('admin.dispatch.confirm',compact('data'));
    }

    public function getReadyToDispatch(Request $request)
    {
        if(Auth::user()->USER_TYPE == 10 ){
            $shop_id  = Auth::user()->SHOP_ID;
            $data['branch'] = DB::table('SA_USER')->where('PK_NO',$shop_id)->pluck('SHOP_NAME', 'PK_NO');
        ...
        return view('admin.dispatch.ready_to_dispatch',compact('data'));
    }
    ...
    public function getDispatchedList(Request $request)
    {
        ...
        return view('admin.dispatch.dispatched',compact('data'));
    }
    ...
    public function getDeliveredList(Request $request)
    {
        ...
        return view('admin.dispatch.delivered',compact('data'));
    }
    public function getCancelRequestList(Request $request)
    {
        if(Auth::user()->USER_TYPE == 10 ){
            $shop_id  = Auth::user()->SHOP_ID;
            $data['branch'] = DB::table('SA_USER')->where('PK_NO',$shop_id)->pluck('SHOP_NAME', 'PK_NO');
        ...
        return view('admin.dispatch.cancel_request',compact('data'));
    }
