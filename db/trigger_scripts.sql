...
DELIMITER $$
 ...
 DROP TRIGGER IF EXISTS  `BEFORE_PRD_ATTRIBUTE_CHILD_INSERT` $$

 CREATE TRIGGER BEFORE_PRD_ATTRIBUTE_CHILD_INSERT
  BEFORE INSERT
  ON PRD_ATTRIBUTE_CHILD FOR EACH ROW
BEGIN
...
DROP TRIGGER IF EXISTS   `AFTER_INV_STOCK_UPDATE` $$

 CREATE TRIGGER AFTER_INV_STOCK_UPDATE AFTER UPDATE ON INV_STOCK FOR EACH ROW BEGIN
declare VAR_TOTAL_FREE_STOCK int(5) default 0;


SELECT COUNT(*) AS TOTAL_FREE_STOCK INTO VAR_TOTAL_FREE_STOCK FROM INV_STOCK WHERE F_BOOKING_NO IS NULL AND ( PRODUCT_STATUS != 420 OR PRODUCT_STATUS IS NULL ) AND F_SHOP_NO = NEW.F_SHOP_NO AND F_PRD_VARIANT_NO = NEW.F_PRD_VARIANT_NO;

UPDATE PRD_VARIANT_STOCK_QTY SET TOTAL_FREE_STOCK = VAR_TOTAL_FREE_STOCK WHERE F_PRD_VARIANT_NO = NEW.F_PRD_VARIANT_NO;
...
DELIMITER $$
DROP TRIGGER IF EXISTS   `AFTER_PRD_BRAND_UPDATE` $$
CREATE  TRIGGER `AFTER_PRD_BRAND_UPDATE` AFTER UPDATE ON `PRD_BRAND` FOR EACH ROW BEGIN
...
DELIMITER $$
DROP TRIGGER IF EXISTS   `AFTER_PRD_MODEL_UPDATE` $$
CREATE  TRIGGER `AFTER_PRD_MODEL_UPDATE` AFTER UPDATE ON `PRD_MODEL` FOR EACH ROW BEGIN
DECLARE VAR_NAME VARCHAR(50) DEFAULT NULL;
...
DELIMITER $$
 ...
 DROP TRIGGER IF EXISTS   `AFTER_PRD_MASTER_SETUP_UPDATE` $$

 CREATE  TRIGGER `AFTER_PRD_MASTER_SETUP_UPDATE` AFTER UPDATE ON `PRD_MASTER_SETUP` FOR EACH ROW
BEGIN

    update INV_STOCK set F_BRAND_NO=NEW.F_BRAND,BRAND_NAME=NEW.BRAND_NAME,F_MODEL_NO=NEW.F_MODEL,MODEL_NAME=NEW.MODEL_NAME
    where F_PRD_VARIANT_NO IN (SELECT PK_NO FROM PRD_VARIANT_SETUP WHERE F_PRD_MASTER_SETUP_NO=NEW.PK_NO)
    and F_BOOKING_NO IS NULL;
...
/* Trigger structure for table `SC_BOX_INV_STOCK_MER` */

DELIMITER $$
 DROP TRIGGER IF EXISTS   `AFTER_SC_BOX_INV_STOCK_MER_INSERT` $$
 CREATE TRIGGER `AFTER_SC_BOX_INV_STOCK_MER_INSERT` AFTER INSERT ON MER_SC_BOX_INV_STOCK FOR EACH ROW BEGIN
...
DELIMITER $$
 DROP TRIGGER IF EXISTS   `AFTER_SC_BOX_INV_STOCK_MER_DELETE` $$
 CREATE TRIGGER `AFTER_SC_BOX_INV_STOCK_MER_DELETE` AFTER DELETE ON MER_SC_BOX_INV_STOCK FOR EACH ROW BEGIN
...
DELIMITER $$
 DROP TRIGGER IF EXISTS   `AFTER_PRD_BEST_SELL_VARIANT_DETAIL_INSERT` $$
 CREATE  TRIGGER `AFTER_PRD_BEST_SELL_VARIANT_DETAIL_INSERT` AFTER INSERT ON `PRD_BEST_SELL_VARIANT_DETAIL` FOR EACH ROW BEGIN

 DECLARE VAR_TOTAL_VARIANT INT DEFAULT 0 ;

SELECT IFNULL(COUNT(*),0) INTO VAR_TOTAL_VARIANT FROM PRD_BEST_SELL_VARIANT_DETAIL
WHERE F_BEST_SELL_MASTER_NO = NEW.F_BEST_SELL_MASTER_NO AND F_PRD_MASTER_NO = NEW.F_PRD_MASTER_NO;

        UPDATE PRD_BEST_SELL_MASTER_DETAIL
        SET TOTAL_BEST_SELL_VARAINT = VAR_TOTAL_VARIANT
        WHERE F_BEST_SELL_MASTER_NO = NEW.F_BEST_SELL_MASTER_NO AND F_PRD_MASTER_NO = NEW.F_PRD_MASTER_NO;
...
DROP TRIGGER IF EXISTS   `BEFORE_SLS_ORDER_INSERT` $$

CREATE  TRIGGER `BEFORE_SLS_ORDER_INSERT` BEFORE INSERT ON `SLS_ORDER` FOR EACH ROW BEGIN
...
IF NEW.ORDER_TYPE_NAME IS NULL THEN
        ...
        ELSE
           SET VAR_ORDER_GROUP_ID = concat(VAR_BOOKING_NO,NEW.F_RESELLER_NO);
        END IF;
...
BEGIN
    ...
    INSERT INTO SLS_BOOKING_DETAILS_AUD (
    PK_NO
    ,F_BOOKING_NO
    ,F_INV_STOCK_NO
    ,COMMENTS
    ...
    ,DISPATCH_NOTIFICATION_FLAG
    ,IS_COD_SHELVE_TRANSFER
    ,COMISSION
    ,RTS_COLLECTION_USER_ID
    ,IS_COLLECTED_FOR_RTS
    ,F_BUNDLE_NO
    ,BUNDLE_SEQUENC
    ,COD_RTC_ACK
    ,LINE_PRICE
    ...
    ,OLD.IS_SM
    ,OLD.REGULAR_PRICE
    ,OLD.INSTALLMENT_PRICE
    ,OLD.IS_REGULAR
    ,OLD.CURRENT_AIR_FREIGHT
    ,OLD.CURRENT_SEA_FREIGHT
    ,OLD.CURRENT_IS_FREIGHT
    ,OLD.CURRENT_SS_COST
    ,OLD.CURRENT_SM_COST
    ,OLD.CURRENT_IS_SM
    ,OLD.CURRENT_REGULAR_PRICE
    ,OLD.CURRENT_INSTALLMENT_PRICE
    ,OLD.CURRENT_IS_REGULAR
    ,OLD.CURRENT_F_DELIVERY_ADDRESS
    ,OLD.ORDER_STATUS
    ,OLD.IS_SELF_PICKUP
    ,OLD.IS_ADMIN_APPROVAL
    ,OLD.IS_READY
    ,OLD.ARRIVAL_NOTIFICATION_FLAG
    ,OLD.DISPATCH_NOTIFICATION_FLAG
    ,OLD.IS_COD_SHELVE_TRANSFER
    ,OLD.COMISSION
    ,OLD.RTS_COLLECTION_USER_ID
    ,OLD.IS_COLLECTED_FOR_RTS
    ,OLD.F_BUNDLE_NO
    ,OLD.BUNDLE_SEQUENC
    ,OLD.COD_RTC_ACK
    ,OLD.LINE_PRICE
...
DELIMITER $$
 ...
 DROP TRIGGER IF EXISTS   `BEFORE_SLS_NOTIFICATION_EMAIL_INSERT` $$

 CREATE  TRIGGER `BEFORE_SLS_NOTIFICATION_EMAIL_INSERT` BEFORE INSERT ON `SLS_NOTIFICATION_EMAIL` FOR EACH ROW BEGIN
...
DROP TRIGGER IF EXISTS   `BEFORE_SLS_SELLERS_INSERT` $$

CREATE  TRIGGER `BEFORE_SLS_SELLERS_INSERT` BEFORE INSERT ON `SLS_RESELLERS` FOR EACH ROW BEGIN
declare VAR_CODE int(11) default 0;
declare VAR_RESELLER_NO int(11) default 0;
...
DELIMITER $$
 DROP TRIGGER IF EXISTS   AFTER_PRD_VARIANT_SETUP_INSERT $$

 CREATE  TRIGGER AFTER_PRD_VARIANT_SETUP_INSERT AFTER INSERT ON PRD_VARIANT_SETUP FOR EACH ROW
BEGIN
...
DELIMITER $$
 DROP TRIGGER IF EXISTS   BEFORE_PRD_VARIANT_STOCK_QTY_UPDATE $$

 CREATE  TRIGGER BEFORE_PRD_VARIANT_STOCK_QTY_UPDATE BEFORE UPDATE ON PRD_VARIANT_STOCK_QTY FOR EACH ROW
BEGIN
DECLARE VAR_TOTAL_FREE_STOCK INT(5) DEFAULT 0;
SELECT COUNT(1) AS TOTAL_FREE_STOCK INTO VAR_TOTAL_FREE_STOCK FROM INV_STOCK WHERE F_BOOKING_NO IS NULL AND F_PRD_VARIANT_NO = NEW.F_PRD_VARIANT_NO;
