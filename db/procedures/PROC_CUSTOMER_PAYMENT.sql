...
DROP PROCEDURE IF EXISTS `PROC_CUSTOMER_PAYMENT`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `PROC_CUSTOMER_PAYMENT`(IN_PK_NO INTEGER(11), IN_TYPE VARCHAR(20))
    NO SQL
BEGIN
  ...
  IF IN_TYPE = 'customer' THEN
    ...
    ELSEIF IN_TYPE = 'reseller' THEN
            ...
            SELECT PK_NO, F_RESELLER_NO,F_PAYMENT_ACC_NO,MR_AMOUNT,PAYMENT_DATE,F_SS_CREATED_BY,PAYMENT_TYPE,PAYMENT_REMAINING_MR INTO VAR_ACC_CUSTOMER_PAYMENTS_PK_NO, VAR_F_CUSTOMER_NO, VAR_F_PAYMENT_ACC_NO, VAR_MR_AMOUNT, VAR_PAYMENT_DATE,VAR_F_SS_CREATED_BY,VAR_PAYMENT_TYPE, VAR_PAYMENT_REMAINING_MR FROM ACC_RESELLER_PAYMENTS WHERE PK_NO = IN_PK_NO;
 ...
 INSERT INTO ACC_BANK_TXN(TXN_REF, TXN_TYPE_IN_OUT, TXN_DATE, AMOUNT_ACTUAL, AMOUNT_BUFFER, IS_CUS_RESELLER_BANK_RECONCILATION, F_ACC_PAYMENT_BANK_NO, F_RESELLER_NO, F_RESELLER_PAYMENT_NO,F_SS_CREATED_BY,SS_CREATED_ON,PAYMENT_TYPE,IS_MATCHED)
...
ELSEIF IN_TYPE = 'merchant' THEN
 ...
 INSERT INTO ACC_BANK_TXN(TXN_REF, TXN_TYPE_IN_OUT, TXN_DATE, AMOUNT_ACTUAL, AMOUNT_BUFFER, IS_CUS_RESELLER_BANK_RECONCILATION, F_ACC_PAYMENT_BANK_NO, F_MERCHANT_NO, F_MERCHANT_PAYMENT_NO,F_SS_CREATED_BY,SS_CREATED_ON,PAYMENT_TYPE,IS_MATCHED)
