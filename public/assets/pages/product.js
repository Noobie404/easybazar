...
/*HScode, Brand, Model by subcategory*/
$(document).on('change','#sub_category', function(){
    var scatId      = $(this).val();
    var url     = $(this).attr('data-url');
    var pageurl = url+'/'+scatId;
    // getHscode(scatId, url);
    getBrandModel(scatId, url);


});


function getBrandModel(scatId, url){
    var id      = scatId;
    var url     = url;
    var pageurl = url+'/'+id;
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.brand != '' ){
                ...
                $('#brand').html("<option value=''>data not found</option>");
            }
            if(data.model != '' ){
                $('#prod_model').html(data.model);
            } else {
                $('#prod_model').html("<option value=''>data not found</option>");
            }
...
function checkMFG(){
    ...
    if(check) {
        ...
        $('#barcode').jqBootstrapValidation();
    } else {
        $('#barcode').attr('readonly',true);
        $('#barcode').attr('required',false);
        $("#barcode").jqBootstrapValidation("destroy");
        // $("#barcode").jqBootstrapValidation({
        //     filter: function () {
