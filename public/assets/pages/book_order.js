        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.status == false){
                    ...
                    if($('input[type=radio][name=payment]:checked').val() == 'azuramart-180'){
                        split_ins('downpayment_180',6);
                    }else{
                        split_ins('downpayment_90',3);
                    }
                }
            },
            complete: function (data){
...
jQuery(document).ready(function($) {
    ...
    $('input[type=radio][id*=regular_price_all],input[type=radio][id*=installmnt_price_all]').change(function() {
        if ($(this).val() == 1) {
            if($('input[type=radio][name=payment]:checked').val() == 'billplz'){
                $('#azuramart_90').attr('checked',false);
                $('#azuramart_180').attr('checked',false);
                $('#billplz').attr('checked',true);
                $('#collapseAzura1').removeClass('show');
                $('#collapseAzura2').removeClass('show');
                $('#collapseOne').addClass('show');
            }else if($('input[type=radio][name=payment]:checked').val() == 'azuramart-90'){
                $('#azuramart_90').attr('checked',true);
                $('#azuramart_180').attr('checked',false);
                $('#billplz').attr('checked',false);
                $('#collapseAzura1').addClass('show');
                $('#collapseAzura2').removeClass('show');
            ...
            $('#billplz').attr('checked',false);
            $('#azuramart_90').attr('checked',false);
            $('#azuramart_180').attr('checked',true);
            $('#collapseOne').removeClass('show');
            $('#collapseAzura1').removeClass('show');
            $('#collapseAzura2').addClass('show');
        }
        count_amount();
        grand_total();
    });
...
function grand_total() {
    ...
    var value = $('input[type=radio][name=price_type_all]:checked').val();
    if (value == 1) {
        // 3 installments
        split_ins('downpayment_90',3);
    }else{
...
$(document).on('click','.payment-card .card-header',function(){
    ...
    if ($(this).find('input[type=radio]').val() == 'billplz' || $(this).find('input[type=radio]').val() == 'azuramart-90') {
        // $('#regular_price_all').attr('checked',true);
        // $('#regular_price_all').prop('checked',true);
        // $('#installmnt_price_all').prop('checked',false);
        // $('#regular_price_all').change();
        $('#regular_price_all').click();
    }else{
        // $('#regular_price_all').prop('checked',false);
        // $('#installmnt_price_all').prop('checked',true);
        // $('#installmnt_price_all').attr('checked',true);
        // $('#installmnt_price_all').change();
        $('#installmnt_price_all').click();
    }
    var value = $('input[type=radio][name=price_type_all]:checked').val();
    if (value == 1) {
        // 3 installments
...
$(document).on('input','#downpayment_90',function() {
    calc_ins_amounts('downpayment_90',3);
})
$(document).on('change keypress paste keyup input','#downpayment_180',function() {
    calc_ins_amounts('downpayment_180',6);
})
function split_ins(id,ins_count) {
    var is_bundle_exists = $('#is_bundle_exists').val();
    if (is_bundle_exists == 1) {
        if($('input[type=radio][name=payment]:checked').val() == 'azuramart-180'){
            var total_price = Number($('#bundle_total_price_ins').val());
        }else{
            var total_price = Number($('#bundle_total_price').val());
        }
    }else{
    ...
    $('#'+id).val(downpayment);
    $('#'+id).attr('min',downpayment);
    $('#'+id).attr('max',total_price);
...
function calc_ins_amounts(id,ins_count) {
    var new_val         = Number($('#'+id).val());//10
    var init_value      = Number($('#'+id).attr('min'));//40
    var p_difference    = new_val-init_value;
    if (init_value >= new_val) {
        var is_bundle_exists = $('#is_bundle_exists').val();
        if (is_bundle_exists == 1) {
            if($('input[type=radio][name=payment]:checked').val() == 'azuramart-180'){
                var total_price = Number($('#bundle_total_price_ins').val());
        ...
        for (var i = 0; i < ins_count+1; i++) {
            if (i > 0) {
                var input_val = Number($('#'+id+'_'+i).attr('data-value'));//53
