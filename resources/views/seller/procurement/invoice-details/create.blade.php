...
@section('title') @lang('invoice_details.new_page_title') @endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Create invoice details</li>
@endsection
...
@push('custom_css')
 ...
 <style type="text/css">
    ...
    .td_line_unit_mer, .td_row_line_total_exvat_actual_mer{display: none;}
 </style>
@endpush('custom_css')
...
@section('content')
    <div class="card card-success">
        <div class="card-header">
            <h4 class="card-title" id="basic-layout-colored-form-control"><i class="ft-plus text-primary"></i> New Invoice Details</h4>
        ...
        <div class="card-content collapse show">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        {!! Form::open([ 'route' => 'seller.invoice-details.store', 'method' => 'post', 'class' => 'form-horizontal', 'files' => true , 'id' => 'save-inv-details']) !!}
                            <div class="form-body">
                                <div class="row">
                                    <div class="col-md-4 pb-1">
                                        <h6><b>Invoice No:</b> {{ $invoice_info->INVOICE_NO }}</h6>
                                        <h6><b>Vendor:</b> {{ $invoice_info->VENDOR_NAME }}</h6>
                                        <h6><b>Primary Discount:</b> {{ $invoice_info->DISCOUNT_PERCENTAGE ?? 0 }}%</h6>
                                        <h6><b>Secondary Discount:</b> {{ $invoice_info->DISCOUNT2_PERCENTAGE ?? 0 }}%</h6>
                                    </div>
                                    <div class="col-md-4 pb-1">
                                        <h6><b>Currency:</b> {{ $invoice_info->INVOICE_CURRENCY }}</h6>
                                        <h6><b>Invoice Exact Value:</b> {{ $invoice_info->INVOICE_EXACT_VALUE }}</h6>
                                        <h6><b>Invoice Exact Vat:</b> {{ $invoice_info->INVOICE_EXACT_VAT }}</h6>
                                        <h6><b>Invoice Exact Postage:</b> {{ $invoice_info->INVOICE_EXACT_POSTAGE }}</h6>
                                        {{-- <h6><b>GBP to MR:</b> {{ $invoice_info->GBP_TO_MR_RATE }}%</h6> --}}
                                    </div>
                                    {{-- <div class="col-md-8">
                                        <div class="row">
                                            <div class="col-md-2 mt-2">
                                                <h6><b>Choose Seller :</b></h6>
                                            </div>
                                            <div class="col-md-4">
                                                {!! Form::select('seller', $data['seller_list'], null, ['class' => 'form-control select2','required']) !!}
                                            </div>
                                        </div>
                                    </div> --}}
                                    <div class="col-md-4">
                                        <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                {{-- <button type="button" class="btn btn-primary  btn-sm search_mother_btn" data-toggle="modal" data-backdrop="true" data-target="#variant-modal" data-url="{{ route('seller.invoice-details.new', [$invoice_info->PK_NO]) }}" >
                                                   Barcode Finder
                                                </button> --}}
                                                <a href="{{ route('seller.invoice-details.new', [$invoice_info->PK_NO]) }}" class="btn btn-primary  btn-sm ">Reset</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-striped table-bordered table-sm" id="invoicetable">
                                                ...
                                                <tbody>
                                                    ...
                                                    @include('seller.procurement.invoice-details._variant_tr',$item)
                                                    @endforeach
                                                    @endif
                                                </tbody>
                                                <tfoot>
 ...
 <script>
    ...
    function typeahead(type) {
        ...
        var engine = new Bloodhound({
            remote: {
                url: get_url+'/seller/product/get-variant-info-like?q=%QUERY%&type='+type,
                wildcard: '%QUERY%',
                cache: false,
            },
    ...
    function getItemList(bar_code)
    {
        if (bar_code != '') {
            ...
            if(check_dupli == 1){
                ...
                var type = `{{ request()->id ? 1 : 0 }}`;
                var pageurl = `{{ URL::to('seller/invoice-details/variant/`+bar_code+`/list/`+type+`')}}`;
                $.ajax({
                    type:'get',
                    url:pageurl,
                    async :true,
...
$(document).on('input','.row_line_unit_mer',function(e){
    var row_id = $(this).data('row_id');
    var row_line_unit = $(this).val();
    var row_line_qty   = $('.line_qty'+row_id).val();
    ...
    $('.line_total_mer'+row_id).val(Number(row_line_unit*row_line_qty).toFixed(2));
    getTotalCol('row_line_total_mer');
...
function getGtotalMer(gtotal_class)
{

    var total_row_line_total_exvat_actual_mer = Number($("#total_row_line_total_exvat_actual_mer").text());
    var total_row_line_vat_actual_mer         = Number($("#total_row_line_vat_actual_mer").text());
    $('#'+gtotal_class).text(Number(total_row_line_total_exvat_actual_mer+total_row_line_vat_actual_mer).toFixed(2));
}
...
function unitPriceEvent(pk_no, type = 'line'){
    ...
    var discount_percentage = parseInt({{ $invoice_info->DISCOUNT_PERCENTAGE ?? 0 }});
    var discount2_percentage = parseInt({{ $invoice_info->DISCOUNT2_PERCENTAGE ?? 0 }});
