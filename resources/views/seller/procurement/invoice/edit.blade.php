...
<li class="breadcrumb-item"><a href="{{ route('seller.invoice') }}">Edit Invoice </a></li>
...
<div class="card card-success min-height">
    ...
    <div class="card-content collapse show">
        <div class="card-body">
            {!! Form::open([ 'route' => ['seller.invoice.update', $invoice->PK_NO], 'method' => 'post', 'class' => 'form-horizontal', 'files' => true , 'novalidate']) !!}
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group {!! $errors->has('vendor') ? 'error' : '' !!}">
                            ...
                            <div class="controls">
                                  <select class="form-control mb-1" required data-validation-required-message="This field is required" tabindex="{{ $tab_index++ }}" id="vendor" name="vendor">
                    ...
                    <div class="col-md-4">
                        <div class="form-group {!! $errors->has('payment_source') ? 'error' : '' !!}">
                            ...
                            <div class="controls">
                                {!! Form::select('payment_source', $accSource, $invoice->F_PAYMENT_SOURCE_NO, [ 'class' => 'form-control mb-1 select2', 'placeholder' => 'Please select', 'data-validation-required-message' => 'This field is required', 'tabindex' =>$tab_index++, 'id' => 'acc_source','required', 'data-url' => URL::to('seller/bank_acc') ]) !!}
                    ...
                    <div class="col-md-4">
                        <div class="form-group {!! $errors->has('store_no') ? 'error' : '' !!}">
                            <label>Payment Method<span class="text-danger">*</span></label>
                            <div class="controls">
                                {!! Form::select('store_no', $stores, $invoice->F_SHOP_NO, [ 'class' => 'form-control mb-1 select2', 'placeholder' => 'Please select','required', 'data-validation-required-message' => 'This field is required', 'tabindex' =>$tab_index++, 'id' => 'store_no']) !!}
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <a href="{{ route('seller.invoice-details.delete', ['id' => $row->PK_NO,'invoice_for' => request()->get('invoice_for') ]) }}" onclick="return confirm('Are You Sure?')" title="INVOICE DELETE"  class="btn btn-xs btn-danger mr-05"><i class="la la-trash"></i></a>
                                                    </td>
                                                    @endif
                                                    @endif
                                                </tr>
            ...
            <div class="form-actions mt-10 text-center">
                <a href="{{ route('seller.invoice')}} ?invoice_for={{ request()->get('invoice_for') }}" class="btn btn-warning mr-1" title="Cancel" ><i class="ft-x"></i> Cancel </a>
...
<script>
   ...
   $(document).on('click','.photo-delete', function(e){
    ...
    var invoice_for = "{{ request()->get('invoice_for') ?? 'easybazar' }}";
    if ('' != id) {
        var pageurl = `{{ URL::to('seller/imvoice_img_delete')}}/`+id+'/seller';
        $.ajax({
            type:'get',
            url:pageurl,
            async :true,
