...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('seller.dashboard') }}">@lang('vendor.breadcrumb_title')</a></li>
                    ...
                    <div class="card card-success">
                        <div class="card-header">
                            @if(hasAccessAbility('new_vendor', $roles))
                                <a class="text-white btn btn-round btn-sm btn-primary" href="{{route('seller.vendor.new')}}" title="Add new vandor"> <i class="ft-plus text-white"></i> @lang('vendor.role_create_btn')</a>
                            @endif
                                            ...
                                            <tr>
                                                ...
                                                <td style="width: 100px;" class="text-center">
                                                    @if(hasAccessAbility('edit_vendor', $roles))
                                                        <a href="{{ route('seller.vendor.edit', [$row->PK_NO]) }}" class="btn btn-xs btn-info" title="Cancel"><i class="la la-edit"></i></a>
                                                    @endif
                                                    @if(hasAccessAbility('delete_vendor', $roles))
                                                        <a href="{{ route('seller.vendor.delete', [$row->PK_NO]) }}" onclick="return confirm('Are You Sure?')" class="btn btn-xs btn-danger" title="Delete"><i class="la la-trash"></i></a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach()
