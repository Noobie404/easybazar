...
$description .= '<div><h6><a href="'.route('admin.order.view', ['id' => $row->BOOKING_PK_NO ?? 0 ]) .'" class="" target="_blank">ORD-'.$row->BOOKING_NO.'</a> <span class="">('. $row->TYPE.') - </span> <span class=""> '. number_format($order_value,2).'</span></h6></div>';
...
if($row->allOrderPayments && count($row->allOrderPayments) > 0 ){
    ...
    foreach ($row->allOrderPayments as $item) {
        $details .= '<li><span class="f-15 fw-b"><a href="'.route('admin.order.view', ['id' => $item->order->F_BOOKING_NO ?? 0 ]) .'" class="" target="_balank" title="Order Date : '.date('d-m-Y',strtotime($item->order->booking->CONFIRM_TIME ?? '')).'">ORD-'. $item->order->booking->BOOKING_NO ?? 0 .'</a> '.number_format($item->PAYMENT_AMOUNT,2).'</span></li>';
    }
    $breakdown .= '<ul class="list-unstyled">'.$details.'</ul>';
}
if(isset($row->allRefunds) && count($row->allRefunds) > 0 ){
    ...
    foreach ($row->allRefunds as $refund) {
        ...
        $details1 .= '<li><div><h6 class="'.$refund_status.'"><a href="'.route('admin.payment.details',[ 'id' => $refund->bankTxn->PK_NO ?? 0 ]).'" class="" target="_blank">PAY-'.$refund->bankTxn->CODE.'</a><span class=""> (Refund) </span><span class="">'.number_format($refund->MR_AMOUNT,2).'</span></h6></div></li>';
    }
    if($details1){
...
$order_value = $row->PAY_AMOUNT;
if($row->PAYMENT_VERIFY == 1){$payment_status = ' text-success'; }else{  $payment_status = ' text-warning';}
$description .= '<div><h6 class="'.$payment_status.'"><a href="'.route('admin.order.view',[ 'id' => $row->F_BOOKING_NO_FOR_PAYMENT_TYPE3 ?? 0 ]).'" class="" target="_blank" title="Payment by AM for order item returned">Partial Payment ORD-'.$row->BOOKING_NO.'</a><span class=""></span><span class=""> - '.number_format($order_value,2).'</span></h6></div>';
...
if($row->amPaymentForOrder ){
    ...
    $item = $row->amPaymentForOrder;
        $details .= '<li><span class="f-15 fw-b"><a href="'.route('admin.order.view', ['id' => $item->PK_NO ?? 0 ]) .'" class="" target="_balank" title="Order Date : '.date('d-m-Y',strtotime($item->CONFIRM_TIME)).'">ORD-'. $item->BOOKING_NO .'</a> '.number_format($row->PAY_AMOUNT,2).'</span></li>';
