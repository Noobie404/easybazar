                            ...
                            <div class="form-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2 pb-1">
                                        <img style="width: 150px !important; height: 150px;" src="{{ fileExit($item->PRIMARY_IMG_RELATIVE_PATH) }}" alt="">
                                    </div>
                                    <div class="col-md-5 pb-1">
                                        ...
                                        if ($string) {
                                            ...
                                            ?>
                                                ...
                                                ?>
                                                X {{ number_format($purchase_price_with_vat,2) }} = {{ number_format($line_price,2) }}
                                                </span>
                                                @else
                                                <?php
                                    ...
                                    <div class="col-md-4 pb-1">
                                        ...
                                        <h5><b>Branch:</b> {{ $item->F_SHOP_NAME }}</h5>
                                        <?php if (Request::segment(4) == 'shelved') {  ?>
                                            <h5><b>Quantity:</b> {{ $item->shelved_qty }}</h5>
                                        <?php } ?>
                                        @if(hasAccessAbility('view_purchace_price', $roles))
                                        @if ($is_vat > 0)
                                        <h5 style="color:#00A65A"><b>Price With VAT </b> {{ number_format($total_price/$total_av_qty,2) }}</h5>
                                        @else
                                        <h5 style="color:red"><b>Price W/VAT :</b> {{ number_format($total_price/$total_av_qty,2) }}</h5>
                                        @endif
                                        <p><small style="color:#ED1C24">*Please add vat and others cost for purchase price</small></p>
                                        <h5><b>Retail Price : </b> {{ number_format($item->REGULAR_PRICE,2) }}</h5>
                                        <h5><b>Special Price : </b> {{ number_format($item->SPECIAL_PRICE,2) }}</h5>
                                        <h5><b>Wholesale Price : </b> {{ number_format($item->WHOLESALE_PRICE,2) }}</h5>
                                        @endif
                                    </div>
