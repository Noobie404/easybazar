...
@elseif($type == 'booked')
    ...
    @else
    <a href="{{ route('admin.order.view',[$item->f_booking_no1]) }}" target="_blank">ORD-{{ $item->BOOKING_NO }}</a> <br>
    @endif
    <strong>Product Count :</strong> {{ $item->order_qty }} <br>
    <strong>Customer :</strong> {{ $item->CUSTOMER_NAME ?? $item->RESHOP_NAME }} ({{ !empty($item->CUSTOMER_NAME) ? 'Customer' : 'Reseller' }}) <br> <br>
