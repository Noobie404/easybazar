...
@if($data['txn']->IS_CUS_RESELLER_BANK_RECONCILATION == 4)
...
<style>
    #scrollable-dropdown-menu .tt-menu{max-height:260px;overflow-y:auto;width:100%;border:1px solid #333;border-radius:5px}.twitter-typeahead{display:block!important}#indextable td{vertical-align:middle}.table-responsive{overflow:visible!important}.picker__holder{max-width:320px!important}.picker__holder table td{padding:0!important}.paymentUpdatefrm{display:none;max-width:320px!important}
...
<div class="card card-success min-height">
    <div class="card-header">
        <h4 class="card-title" id="basic-layout-colored-form-control"><i class="ft-eye text-primary"></i> {{ $data['txn']->IS_CUS_RESELLER_BANK_RECONCILATION == 4 ? 'Merchant' : '' }} Payment</h4>
            ...
            <table class="table table-striped table-bordered table-sm">
                ...
                <tbody>
                    <tr>
                        ...
                        <td stye="width:33%" class="text-right">Entered By : <span style="text-transform: uppercase">{{ $data['txn']->createdBy->NAME ?? '' }}</span></td>
                    </tr>
                    <tr>
                        <td stye="width:33%">Deposit Amount : {{ number_format($data['txn']->NAME,2) }}</td>
                        <td stye="width:33%">
                    ...
                    <tr>
                        <td stye="width:33%">
                            <P>Order Assigned :
                                @if($data['txn']->F_CUSTOMER_PAYMENT_NO)
                                    {{ number_format($data['txn']->customerPayment->allOrderPayments->sum('PAYMENT_AMOUNT') ?? 0,2) }}
                                @elseif($data['txn']->F_RESELLER_PAYMENT_NO)
                                    {{ number_format($data['txn']->resellerPayment->allOrderPayments->sum('PAYMENT_AMOUNT') ?? 0,2) }}

                                @elseif($data['txn']->F_MERCHANT_PAYMENT_NO)
                                    <span>{{ number_format($data['txn']->MR_AMOUNT - $data['txn']->PAYMENT_REMAINING_MR,2)  }}</span>
                                @endif
                            </P>

                                @if($data['txn']->F_CUSTOMER_PAYMENT_NO)
                                    @if(isset($data['txn']->TRANS_TO) && count($data['txn']->TRANS_TO) > 0 )
                                    <P>Amount Transferred :
                                        @foreach($data['txn']->TRANS_TO as $tk => $txn_to )
                                            {{ number_format($txn_to->AMOUNT,2) }} <a href="{{ route('admin.customer.view',$txn_to->F_TO_CUSTOMER) }}">(CUST-{{ $txn_to->CUSTOMER_NO }})</a>
                                        @endforeach
                                    </P>
                                @endif
                                ...
                                @elseif($data['txn']->F_RESELLER_PAYMENT_NO)
                                    ...
                                    <p>
                                        @foreach($data['txn']->TRANS_TO as $tk => $txn_to )
                                            {{ number_format($txn_to->AMOUNT,2) }} <a href="{{ route('admin.customer.view',$txn_to->F_TO_RESELLER_NO) }}">(CUST-{{ $txn_to->RESELLER_NO }})</a>
                                        @endforeach
                                    </p>
                                    @endif
                        ...
                        <td stye="width:33%">Paid By :
                            ...
                            @elseif($data['txn']->F_MERCHANT_PAYMENT_NO)
                                {{ $data['txn']->merchantPayment->PAID_BY }}
                            @endif
                    ...
                    <tr>
                        <td stye="width:33%">Credit Amount :
                            ...
                            @elseif($data['txn']->F_MERCHANT_PAYMENT_NO)
                            {{ number_format($data['txn']->merchantPayment->PAYMENT_REMAINING_MR,2) }}
                            @endif
                        ...
                        <td stye="width:33%" >Tx Ref :
                            ...
                            @elseif($data['txn']->F_MERCHANT_PAYMENT_NO)
                                {{ $data['txn']->merchantPayment->SLIP_NUMBER }}
                            @endif
                        ...
                        @if(isset($data['order_payments']) && count($data['order_payments']) > 0 )
                            @foreach($data['order_payments'] as $key =>  $row)
                                <tr title="order_pk : {{ $row->order->PK_NO }}">
                                    ...
                                    <td>
                                        {{ date('d-m-Y h:i A',strtotime($row->order->booking->CONFIRM_TIME)) }}
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        @if(hasAccessAbility('delete_orderpayment', $roles) && !isset($row->order->innstallmentPayment))
