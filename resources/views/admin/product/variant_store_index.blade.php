...
@section('page-name') Product variant @endsection
@push('custom_css')
                ...
                <div class="col-md-12">
                    <div class="card card-sm card-success">
                        <div class="card-header">
                            ...
                            {{-- <a class="btn btn-sm btn-info text-white" href="{{ route('seller.product.variant.store.index',$id) }}?product=all" title="PRODUCT VARIANT"> Variant product (EB)</a>
                            <a class="btn btn-sm btn-info text-white" href="{{ route('seller.product.variant.store.index',$id) }}?product=shop" title="PRODUCT VARIANT"> Variant product (Shop)</a> --}}
...
<script>
    ...
    function callDatatable() {
        ...
        var table = $('#process_data_table').DataTable({
            ...
            // iDisplayStart: value,
            ajax: {
                url: get_url+'/store-product-master-variant-list',
                type: 'POST',
                data: function(d) {
                    d._token = "{{ csrf_token() }}";
                    d.status = status;
                    d.product = product;
                    d.product_master = product_master;
                }
            },
            columns: [
                {
        ...
        var product = `{{ request()->get('product') }}`;
            $('#rows_ [id*=product_check]').focus(function() {
                ...
                if (success) {
                    ...
                    $.ajax({
                        type:'post',
                        url: get_url+'/ajax/if-product-variant-in-shop',
                        data:{
