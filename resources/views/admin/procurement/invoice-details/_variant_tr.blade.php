...
<tr class="row_{{$item->PK_NO}}">
    ...
    <td class="">
        ...
        <input class="form-control input-sm mb-1" placeholder="Enter invoice name" @if(!empty($item->VARIANT_CUSTOMS_NAME)) value="{{ $item->VARIANT_CUSTOMS_NAME }}" @else value="{{ $item->VARIANT_NAME }}" @endif tabindex="1" name="invoice_name[]" type="text" data-validation-required-message = "This field is required" required data-row_id="{{ $item->PK_NO }}">
    </td>
    @endif
    <td class="">
