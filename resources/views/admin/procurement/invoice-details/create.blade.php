...
@push('custom_css')
 ...
 <style type="text/css">
    ...
    .disabled{pointer-events: none;background-color:#f9eddd!important;opacity: 1;}
    .enabled{cursor: pointer;opacity: 1;}
    .list-group-item{position: relative;}
    .item-notice{position:absolute;right:0;top:0;font-size:10px;}
 </style>
@endpush('custom_css')
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="hidden" name="shop_id" id="shop_id" value="{{$invoice_info->F_SHOP_NO}}" class="" />


                            <div class="form-body">
                                <div class="row">
                                    <div class="col-md-4 pb-1">
                                        <h6><b>Shop Name:</b> <a href="">{{ $invoice_info->SHOP_NAME }}</a></h6>
                                        <h6><b>Invoice No:</b> {{ $invoice_info->INVOICE_NO }}</h6>
                                        <h6><b>Vendor:</b> <a href="">{{ $invoice_info->VENDOR_NAME }}</a></h6>
 ...
 <script>
    ...
    function typeahead(type) {
        ...
        var engine = new Bloodhound({
            remote: {
                url: get_url+'/product/get-variant-info-like?q=%QUERY%&type='+type+'&shop_id='+shop_id,
                wildcard: '%QUERY%',
                cache: false,
            },
        ...
        $(".search-input2").typeahead({
            ...
            templates: {
                ...
                suggestion: function (data) {
                    return '<span class="list-group-item ' + data.IS_ASSIGNED + '" ' + data.IS_ASSIGNED + ' title="' + data.NAME + '" style="position:relative;"><span class="item-notice mr-1">' + data.MSG + '</span><img style="width:40px;" class="mr-1" src="'+data.PRIMARY_IMG_RELATIVE_PATH+'" alt=" ">' + data.NAME + '</span>'
                }
            }
        });
    }
    ...
    function getItemList(bar_code)
    {
        if (bar_code != '') {
            ...
            if(check_dupli == 1){
                ...
                var pageurl = `{{ URL::to('invoice-details/variant/`+bar_code+`/list/`+type+`')}}`;
                $.ajax({
                    type:'get',
                    url:pageurl,
                    async :true,
