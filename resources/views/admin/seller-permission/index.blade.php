                    ...
                    <div class="card card-success">
                        <div class="card-header">
                            @if(hasAccessAbility('new_action', $roles))
                                <a class="text-white btn btn-round btn-sm btn-primary" title="Add new" href="{{route('seller.permission.new')}}"><i class="ft-plus text-white"></i> @lang('admin_action.action_create_btn')</a>
                            @endif
                                    ...
                                    <table class="table table-striped table-bordered alt-pagination" id="indextable">
                                        ...
                                        @foreach($rows as $row)
                                            <tr>
                                                ...
                                                <td>
                                                    @if(hasAccessAbility('edit_seller_action', $roles))
                                                        <a class="text-white btn btn-xs btn-primary mr-1" title="Edit" href="{{ route('seller.permission.edit', array($row->PK_NO)) }}"><i class="la la-edit"></i></a>
                                                    @endif
                                                    @if(hasAccessAbility('delete_seller_action', $roles))
                                                        <a class="text-white btn btn-xs btn-danger mr-1" title="Delete" href="{{ route('seller.permission.delete', array($row->PK_NO)) }}"  onclick="return confirm('Are you sure?')"><i class="la la-trash"></i></a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach()
