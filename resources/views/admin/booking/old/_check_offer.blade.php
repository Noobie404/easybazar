                ...
                <table class="table table-striped table-bordered " id="offerTbl">
                    ...
                    <tbody>
                        @if(isset($data['bundle']) && count($data['bundle']))
                            {!! Form::hidden('', 1, ['id'=>'is_bundle_exists']) !!}
                            @foreach($data['bundle'] as $key => $row)
                        @php
                        $total_bundle = 0;
                        $total_bundle_ins = 0;

                        $total_bundle_ins += $row->TOTAL_INSTALLMENT_BUNDLE_PRICE;
                        if($is_regular == 1){
                            $total_bundle += $row->TOTAL_REGULAR_BUNDLE_PRICE;
                        }else{
                            $total_bundle += $row->TOTAL_INSTALLMENT_BUNDLE_PRICE;
                        }
                        if($is_freight == 1 ){
                            $total_bundle += ($row->P_AIR+$row->R_AIR)*$row->BUNDLE_QTY;
                            $total_bundle_ins += ($row->P_AIR+$row->R_AIR)*$row->BUNDLE_QTY;
                        }elseif($is_freight == 2 ){
                            $total_bundle += ($row->P_SEA+$row->R_SEA)*$row->BUNDLE_QTY;
                            $total_bundle_ins += ($row->P_SEA+$row->R_SEA)*$row->BUNDLE_QTY;
                        }else{
                            $total_bundle += 0;
                            $total_bundle_ins += 0;
                        }
                        ...
                        <tr class="bg-bundle bundle-summary">
                            ...
                            <td style="width: 10%" class="text-right">
                                @php
                                $grand_total += $total_bundle;
                                $grand_total_ins += $total_bundle_ins;
                                @endphp
                        ...
                        <?php
                            $total_non_bundle = 0;
                            $total_non_bundle_ins = 0;

                            $total_non_bundle_ins += $nrow->CURRENT_INSTALLMENT_PRICE*$nrow->ITEM_QTY;
                            if($is_regular == 1){
                            ...
                            if($is_freight == 1 ){
                                ...
                                $total_non_bundle_ins += $nrow->AIR_FREIGHT*$nrow->ITEM_QTY;
                            }if($is_freight == 2 ){
                                    $total_non_bundle += $nrow->SEA_FREIGHT*$nrow->ITEM_QTY;
                                    $total_non_bundle_ins += $nrow->SEA_FREIGHT*$nrow->ITEM_QTY;
                            }else{
                                $total_non_bundle += 0;
                                $total_non_bundle_ins += 0;
                            }
                        ...
                        <tr>
                            ...
                            <td style="width: 10%" class="text-right">
                                @php
                                    $grand_total += $total_non_bundle;
                                    $grand_total_ins += $total_non_bundle_ins;
                                @endphp
                        ...
                        <tr>
                            ...
                            <td colspan="3" class="text-right">
                                ...
                                <input type="hidden" name="total_price" id="bundle_total_price" value="{{ $grand_total }}" />
                                <input type="hidden" name="total_price_ins" id="bundle_total_price_ins" value="{{ $grand_total_ins }}" />
                            </td>
