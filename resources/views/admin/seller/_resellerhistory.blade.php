...
$description .= '<div><h6><a href="'.route('admin.order.view', ['id' => $row->BOOKING_PK_NO ]) .'" class="" target="_blank">ORD-'.$row->BOOKING_NO.'</a> <span class="">('. $row->TYPE.') - </span> <span class=""> '. number_format($order_value,2).'</span></h6></div>';
...
$description .= '<div><h6 class="'.$payment_status.'"><a href="'.route('admin.order.view',[ 'id' => $row->F_BOOKING_NO_FOR_PAYMENT_TYPE3 ]).'" class="" target="_blank" title="Payment by AM for order item returned">Partial Payment ORD-'.$row->BOOKING_NO.'</a><span class=""></span><span class=""> - '.number_format($order_value,2).'</span></h6></div>';
