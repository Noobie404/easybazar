                    ...
                    <div class="card card-success">
                        <div class="card-header">
                            @if(hasAccessAbility('new_user_group', $roles))
                                <a class="text-white btn btn-round btn-sm btn-primary" href="{{route('seller.user-group.new')}}" title="Add new user group"><i class="ft-plus text-white"></i> @lang('user_group.btn_menu_create')</a>
                            @endif
                                    ...
                                    <table class="table table-striped table-bordered alt-pagination" id="indextable">
                                        ...
                                        @foreach($rows as $row)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('seller.role.edit', [$row->role_pk]) }}" target="_blank">
                                                        {{$row->NAME}}
                                                    </a>
                                                </td>
                                                ...
                                                <td>
                                                    @if(hasAccessAbility('edit_user_group', $roles))
                                                        <a class="text-white btn btn-xs btn-primary mr-1" href="{{ route('seller.user-group.edit', [$row->PK_NO]) }}" title="Edit"><i class="la la-edit"></i></a>
                                                    @endif
                                                    @if(hasAccessAbility('delete_user_group', $roles))
                                                        <a class="text-white btn btn-xs btn-danger mr-1" href="{{ route('seller.user-group.delete', [$row->PK_NO]) }}" title="Delete"><i class="la la-trash"></i></a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach()
