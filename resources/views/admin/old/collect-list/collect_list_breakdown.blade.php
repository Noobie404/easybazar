...
if($row->TYPE == 'Customer' || $row->TYPE == 'Reseller'){
    ...
    $description .= '<div><h4><a href="'.route('admin.order.view', ['id' => $row->BOOKING_PK_NO ]) .'" class="link" target="_blank">ORD-'.$row->BOOKING_NO.'</a> <span class="text-danger">('. $row->TYPE.' Order) - </span> <span class="text-success">(RM) '. number_format($order_value,2).'</span></h4></div>';
