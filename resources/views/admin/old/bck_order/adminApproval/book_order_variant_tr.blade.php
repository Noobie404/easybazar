...
<tr>
    <td style="width: 10%;" class="img_td">
        ...
        @else
        <div class="lightgallery" style="margin:0px  auto; text-align: center; ">
            <a class="img_popup " href="{{ asset('assets/images/no_image.jpg') }}"  title=""><img style="width: 150px !important; height: 150px;" data-src="{{ asset('assets/images/no_image.jpg') }}" alt="" src="{{asset($item['info'][0]->PRIMARY_IMG_RELATIVE_PATH)}}" class="unveil"></a>
        </div>
        @endif
    ...
    <p>BARCODE : <span>{{$item['info'][0]->BARCODE ?? ''}}</span></p>
    ...
    @endif
    <p>SM- <span class="danger"> {{number_format($item['info'][0]->INV_SM_COST,2, '.', '') }}</span> SS- <span class="danger"> {{number_format($item['info'][0]->INV_SS_COST,2, '.', '') }}</span></p>
</td>
    <th style="width: 10%">
        <table id="warehouse">
            @if(!empty($item['info']))
            ...
            <tr id="warehouse_delete{{ $data->PK_NO }}" data-inv_pk="{{ $data->PK_NO }}" style="cursor: pointer;">
                <th title="{{ isset($data->BOX_TYPE) ? "BOX TYPE-($data->BOX_TYPE)" : '' }} {{ isset($data->SHIPMENT_TYPE) ? "SHIPMENT TYPE-($data->SHIPMENT_TYPE)" : '' }}" style="{{ $data->ORDER_STATUS >= 80 ? 'opacity:.5' : '' }}" >
                    <?php
    ...
    <th id="postage_costs_th" style="width: 3%">
        ...
        <div id="postage_delete{{ $data->PK_NO }}">
            <div style="margin-bottom: 5px" id="postage_costs">
                ...
                <input name="postage_costs[]" id="single_postage_value" type="number" style="font-weight: normal;font-size: 12px;display: inline-block;text-align:right;width: 40px;" class="form-control input-sm" {{ $data->ORDER_STATUS >= 80 ? 'readonly' : '' }}>
                {{-- @if ($data->CURRENT_IS_SM != $data->IS_SM)
                <span style="text-decoration-line: line-through;font-size:13px;">{{ $data->IS_SM == 1 ? 'SM ' : 'SS ' }}</span>
                @endif --}}
                @if (($data->SM_COST != $data->CURRENT_SM_COST || $data->SS_COST != $data->CURRENT_SS_COST) && $data->IS_ADMIN_APPROVAL == 1)
                <span style="text-decoration-line: line-through;font-size:13px;">{{ $data->IS_SM == 1 ? 'SM :' : 'SS ?' }}{{ $data->CURRENT_SM_COST != $data->SM_COST ? $data->SM_COST : $data->SS_COST }}</span>
                @endif
            </div>
        </div>
        @endforeach
    ...
    <th id="freight_costs_th" style="width: 7%">
        ...
        <div id="freight_delete{{ $data->PK_NO }}" style="width: 70%">
            <div style="margin-bottom: 5px;text-align: center" id="freight_costs">
                ...
                <input name="freight_costs[]" style="text-align:right;display: inline-block;width: 50px;" type="number" class="form-control input-sm ml-2" id="per_product_freight_value" {{ $data->ORDER_STATUS >= 80 ? 'readonly' : '' }}>
                @if (($data->CURRENT_AIR_FREIGHT != $data->AIR_FREIGHT || $data->CURRENT_SEA_FREIGHT != $data->SEA_FREIGHT) && $data->IS_ADMIN_APPROVAL == 1)
                <span style="text-decoration-line: line-through;font-size:13px;">{{ $data->CURRENT_AIR_FREIGHT != $data->AIR_FREIGHT ? $data->AIR_FREIGHT : $data->SEA_FREIGHT }}</span>
                @endif
            </div>
        </div>
        @endforeach
    ...
    <th id="per_product_costs_th" style="width: 1%;">
        ...
        <div id="per_product_costs_delete{{ $data->PK_NO }}" style="width: 100%;">
            <div style="margin-bottom: 5px" id="per_product_costs">
                ...
                <input name="unit_costs[]" id="per_product_value" type="number" style="font-weight: normal;font-size: 12px;text-align:right;display: inline-block;width: 50px;" class="form-control input-sm" {{ $data->ORDER_STATUS >= 80 ? 'readonly' : '' }}>
                @if (($data->CURRENT_REGULAR_PRICE != $data->BOOK_REGULAR_PRICE || $data->CURRENT_INSTALLMENT_PRICE != $data->BOOK_INSTALLMENT_PRICE) && $data->IS_ADMIN_APPROVAL == 1)
                <span style="text-decoration-line: line-through;font-size:13px;">{{ $data->CURRENT_REGULAR_PRICE != $data->BOOK_REGULAR_PRICE ? $data->BOOK_REGULAR_PRICE : $data->BOOK_INSTALLMENT_PRICE }}</span>
                @endif
            </div>
        </div>
        @endforeach
    ...
    <th class="text-center" style="width: 5%" id="action_col">
        @foreach ($item['info'] as $data)
        <div style="display: flex;margin-bottom: 9px;width: 50%;" id="action_column{{ $data->PK_NO}}">
