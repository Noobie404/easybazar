                ...
                @if($book_detail->F_BUNDLE_NO == $offer->bundle->PK_NO && $offer->BUNDLE_SEQUENC == $book_detail->BUNDLE_SEQUENC )
                    ...
                    <tr class="bg-bundle-item ">
                        <td class="img_td">
                            ...
                            @else
                            <div class="lightgallery" style="margin:0px  auto; text-align: center; ">
                                <a class="img_popup " href="{{ asset('assets/images/no_image.jpg') }}"  title=""><img style="width: 40px !important; height: 40px;" data-src="{{ asset('assets/images/no_image.jpg') }}" alt="" src="{{asset($vphoto->RELATIVE_PATH)}}" class="unveil"></a>
                            </div>
                            @endif
                        ...
                        <td>
                            @if ($book_detail->stock->IS_FAULTY == 1)
                            <i class="la la-warning" title="FAULTY"></i>
                            @endif
                            {{ $book_detail->stock->warehouse->NAME ?? '' }}</td>
            ...
            @foreach( $bookingDetails as $key => $book_detail )
                @if($book_detail->F_BUNDLE_NO == null)
                    ...
                    <tr >
                        ...
                        <td><div id="warehouse_delete_{{  $book_detail->PK_NO }}" data-inv_pk="{{  $book_detail->F_INV_STOCK_NO }}">
                            @if ($book_detail->stock->IS_FAULTY == 1)
                            <i class="la la-warning" title="FAULTY"></i>
                            @endif
                            {{ $book_detail->stock->warehouse->NAME ?? '' }}</div></td>
                        <td><input class="form-control form-control-sm input-sm max_val_check text-center"  type="number" min="1" max="1" value="1" readonly></td>
                        <td>
                        ...
                        <td>
                            @if ($book_detail->ORDER_STATUS <= 60 || $book_detail->ORDER_STATUS == null )
                                <a href="javascript:void(0)" id="delete_single_prd{{ $book_detail->F_INV_STOCK_NO }}" class="btn btn-xs btn-danger mr-1" data-delete_id="{{ $book_detail->F_INV_STOCK_NO }}" style="float: left;{{ $book_detail->ORDER_STATUS == 60 ? 'display:none' : 'display:block' }}" title="DELETE"  data-type="nonbundle" data-booking-no="{{ $book->PK_NO }}" ><i class="la la-trash"></i></a>
                            @endif
                        </td>
                    </tr>
