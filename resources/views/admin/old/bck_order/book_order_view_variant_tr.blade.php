...
?>
<tr class="{{ isset($item['info'][0]->ID) ? 'bg-return-item' : '' }}">
    <td style="width: 10%;" class="img_td {{ isset($item['info'][0]->ID) ? 'bg-return-item' : '' }}">
        ...
        <table id="warehouse">
            ...
            <tr id="warehouse_delete{{ $data->PK_NO }}">
                <th title="{{ isset($data->BOX_TYPE) ? "BOX TYPE-($data->BOX_TYPE)" : '' }} {{ isset($data->SHIPMENT_TYPE) ? "SHIPMENT TYPE-($data->SHIPMENT_TYPE)" : '' }}" style="{{ $data->ORDER_STATUS >= 80 ? 'opacity:.5' : '' }}">
                    ...
                    <strong>
                        @if ($data->IS_FAULTY == 1)
                        <i class="la la-warning" title="FAULTY"></i>
                        @endif
                        {{ $data->SHIPMENT_STATUS >= 20 ? $status[$data->SHIPMENT_STATUS] : $data->INV_WAREHOUSE_NAME }}
            ...
            <tr id="warehouse_delete{{ $data->PK_NO }}">
                <th style="{{ $data->ORDER_STATUS >= 80 ? 'opacity:.5' : '' }}">
                    @if ($data->IS_FAULTY == 1)
                    <i class="la la-warning" title="FAULTY"></i>
                    @endif
                    {{ $data->INV_WAREHOUSE_NAME }}<strong> (Ready Stock)</strong>
                </th>
            </tr>
            @endif
            @endforeach
    ...
    <th id="postage_costs_th" style="width: 3%">
        ...
        <div id="postage_delete{{ $data->PK_NO }}">
            <div style="margin-bottom: 5px" id="postage_costs">
                ...
                <input name="postage_costs[]" id="single_postage_value" type="number" style="font-weight: normal;font-size: 12px;display: inline-block;text-align:right;" class="form-control input-sm" readonly>
            </div>
        </div>
        @endforeach
    </th>
    <th id="freight_costs_th" style="width: 7%">
        ...
        <div id="freight_delete{{ $data->PK_NO }}" style="width: 70%">
            <div style="margin-bottom: 5px;text-align: center" id="freight_costs">
                ...
                <input name="freight_costs[]" style="text-align:right;" type="number" class="form-control input-sm ml-2" id="per_product_freight_value" readonly>
            </div>
        </div>
        @endforeach
    </th>
    <th id="per_product_costs_th" style="width: 1%;">
        ...
        <div id="per_product_costs_delete{{ $data->PK_NO }}" style="width: 100%;">
            <div style="margin-bottom: 5px" id="per_product_costs">
                ...
                <input name="unit_costs[]" id="per_product_value" type="number" style="font-weight: normal;font-size: 12px;text-align:right;" class="form-control input-sm" readonly>
            </div>
        </div>
        @endforeach
    </th>
        ...
        <div id="checkbox_delete{{ $data->PK_NO }}" style="width: 5%">
            <div style="margin-bottom: 11.5px" id="checkbox_div" class="skin skin-square">
                <fieldset>
                    ...
                    <input name="checkbox_value_{{ $data->PK_NO }}" type="checkbox" data-pk_no="{{ $data->PK_NO }}" id="checkbox_of_order" disabled>
                </fieldset>
            </div>
        </div>
        @endforeach
    ...
    <th id="selfpickup_th" style="width: 1%;">
        ...
        <div id="selfpickup_delete{{ $data->PK_NO }}" style="width: 5%">
            <div style="margin-bottom: 11.5px" id="selfpickup_div" class="skin skin-flat">
                <fieldset>
                    {{-- {{Form::hidden('checkbox_value[]','off')}} --}}
                    <input name="selfpickup_value_{{ $data->PK_NO }}" type="checkbox" data-pk_no="{{ $data->PK_NO }}" id="selfpickup_of_order" disabled {{ $data->IS_SELF_PICKUP == 1 ? 'checked' : '' }}>
                </fieldset>
            </div>
        </div>
        @endforeach
