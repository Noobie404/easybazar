...
<tr>
            ...
            <tr id="warehouse_delete{{ $data->PK_NO }}" data-inv_pk="{{ $data->PK_NO }}" style="cursor: pointer;">
                <th title="{{ isset($data->BOX_TYPE) ? "BOX TYPE-($data->BOX_TYPE)" : '' }} {{ isset($data->SHIPMENT_TYPE) ? "SHIPMENT TYPE-($data->SHIPMENT_TYPE)" : '' }}@if($data->F_BUNDLE_NO), Offer : {{ $data->BUNDLE_NAME}} @endif " style="{{ $data->ORDER_STATUS >= 80 ? 'opacity:.5' : '' }}" >
                    ...
                    <strong>
                        ...
                        ({{ $data->SHIPMENT_STATUS >= 20 ? Carbon::parse(Carbon::now())->diffInDays($data->SCH_ARRIVAL_DATE,false) : '90' }} Days)
                        </strong>
    ...
    <th class="text-center" style="width: 5%" id="action_col">
        @foreach ($item['info'] as $data)
        <div style="display: flex;margin-bottom: 5px;width: 50%;" id="action_column{{ $data->PK_NO}}">
            {!! Form::hidden('INV_PK_NO[]', $data->F_INV_STOCK_NO ) !!}
            @if (($data->ORDER_STATUS <= 60 || $data->ORDER_STATUS == null) && ($data->F_BUNDLE_NO == null))
            <a href="javascript:void(0)" id="delete_single_prd{{ $data->PK_NO }}" class="btn btn-xs btn-danger mr-1" data-delete_id="{{ $data->PK_NO }}" style="float: left;{{ $data->ORDER_STATUS == 60 ? 'display:none' : 'display:block' }}" title="DELETE"  data-type="nonbundle" data-booking-no="{{ $bookNo ?? null }}" ><i class="la la-trash"></i></a>
            @endif
        </div>
        @endforeach
    </th>
