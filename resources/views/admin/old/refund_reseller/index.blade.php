...
<div class="card card-success">
    <div class="card-content collapse show">
        <div class="card-body" style="padding: 15px 5px;">
            @include('admin.refund_reseller._header')
            <hr>
...
<div class="modal fade text-left" id="refundRequestModal" tabindex="-1" role="dialog" aria-labelledby="refundRequestModal" aria-hidden="true" >
    <div class="modal-dialog modal-md" role="document">
            {!! Form::open([ 'route' => 'admin.reseller.refundrequeststore', 'method' => 'post', 'class' => 'form-horizontal', 'files' => false, 'id' => 'refundrequestFrm']) !!}
            {!! Form::hidden('reseller_no',null,['id'=> 'reseller_no']) !!}
            {!! Form::hidden('is_customer',0,['id'=> 'is_customer']) !!}
...
$(document).on('click','.refundRequest',function(){
    ...
    var reseller = $(this).data('name');
    var reseller_no = $(this).data('reseller_no');
    $('#reseller_no').val(reseller_no);
    $('#c_name').text(reseller);
...
$(document).ready(function() {
    var table   = $('#process_data_table').DataTable({
            ...
            autoWidth: false,
            ajax: {
                url: get_url+'/reseller/refundlist',
                type: 'POST',
                data: function(d) {
                    d._token        = "{{ csrf_token() }}";
                }
