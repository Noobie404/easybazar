                ...
                <div class="col-12">
                    <div class="card card-success">
                        <div class="card-header">
                            @if(hasAccessAbility('new_seller_menu', $roles))
                            <a class="text-white btn btn-round btn-sm btn-primary" href="{{route('seller.permission-group.new')}}" title="Add new"><i class="ft-plus text-white"></i> @lang('admin_action.btn_action_create')</a>
                            @endif
                                    ...
                                    <table class="table table-striped table-bordered alt-pagination" id="indextable">
                                        ...
                                        @foreach($rows as $row)
                                            <tr>
                                                ...
                                                <td>{{$row->ORDER_ID}}</td>
                                                <td>
                                                    @if(hasAccessAbility('edit_seller_menu', $roles))
                                                        <a class="text-white btn btn-xs btn-primary mr-1" href="{{ route('seller.permission-group.edit', ['id' => $row->PK_NO] ) }}" title="Edit"> <i class="la la-edit"></i></a>
                                                    @endif
                                                    @if(hasAccessAbility('delete_seller_menu', $roles))
                                                        <a class="text-white btn btn-xs btn-danger mr-1" href="{{ route('seller.permission-group.delete', array($row->PK_NO)) }}" title="Delete"><i class="la la-trash"></i></a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach()
