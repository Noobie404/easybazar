...
@section('title') @lang('admin_role.edit_page_title') @endsection
@section('page-name') @lang('admin_role.list_page_sub_title') @endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{url('/admin.admin-user')}}">@lang('admin_role.edit_page_breadcrumb_title_1')</a></li>
    <li class="breadcrumb-item"><a href="{{ route('admin.role') }}">@lang('admin_role.edit_page_breadcrumb_title_2')</a></li>
    <li class="breadcrumb-item active">@lang('admin_role.edit_page_breadcrumb_title_active')</li>
@endsection
            ...
            <div class="card-content collapse show">
                <div class="card-body">
                    @if ($errors->any())
                        ...
                        @endforeach
                    @endif
                    {!! Form::open([ 'route' => ['admin.role.update', $role->PK_NO] , 'method' => 'post', 'class' => 'form-horizontal', 'files' => true , 'novalidate']) !!}
                    @csrf
                    <div class="form-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="controls">
                                        {!! Form::text('role_name',$role->NAME, [ 'class' => 'form-control', 'data-validation-required-message' => 'This field is required', 'placeholder' => 'Enter group name', 'tabindex' => 1 ]) !!}
                                    </div>
                                ...
                                <tr>
                                    ...
                                    <td class="skin skin-flat" style="text-align: left;">
                                        <fieldset style="margin-left: 68px;">
                                            @foreach($group->permissions as $i => $permissions)
                                                @if(in_array($group->permissions[$i]->NAME, $assigned_perms) && $group->permissions[$i]->NAME != '')
                                                    <label class="m-checkbox" for="input-{{ $loop->parent->index+1}}{{ $i }}" style="margin-left: 40px;" title="{{ $permissions->NAME }}">
                                                        <input name="permission[]" type="checkbox" checked="checked"
                                                               class="icheckbox_flat-pink checked"
                                                               value="{{ $group->permissions[$i]->NAME }}"
