    ...
    <div class="card card-success min-height">
        ...
        <div class="card-content collapse show">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group {!! $errors->has('shipment_for') ? 'error' : '' !!}">
                            <div class="controls">
                                <label>Shipment for<span class="text-danger">*</span></label>
                                <div class="controls">
                                    {!! Form::select('shipment_for', $shipemt_for, $data['data'] != null ? $data['data']->SHIPMENT_FOR : null, ['class'=>'form-control mb-1','id' => 'shipment_for', 'data-validation-required-message' => 'This field is required']) !!}
                                    {!! $errors->first('shipment_for', '<label class="help-block text-danger">:message</label>') !!}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 merchant_no_div">
                        <div class="form-group {!! $errors->has('merchant_no') ? 'error' : '' !!}">
                            <div class="controls">
                                <label>Merchant<span class="text-danger">*</span></label>
                                <div class="controls">
                                    {!! Form::select('merchant_no', $merchants, $data['data'] != null ? $data['data']->F_MERCHANT_NO : null, ['class'=>'form-control mb-1','id' => 'merchant_no']) !!}
                                    {!! $errors->first('merchant_no', '<label class="help-block text-danger">:message</label>') !!}
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('custom_js')
    ...
    <script>
        ...
        $(document).on('change', '#shipment_for' , function(e){           
            changeShipmentFor($(this).val());
        })   
        changeShipmentFor($('#shipment_for').val());        
        function changeShipmentFor(shipment_for){
            if(shipment_for == 2){
                $('.merchant_no_div').show();
            }else{
                $('.merchant_no_div').hide();
            }
        }
        </script>
@endpush('custom_js')
