                                    ...
                                    <table class="table table-striped table-bordered alt-pagination table-sm" id="indextable">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{ $loop->index+1 }}</td>
                                                <td>
                                                    {{ $row->SHIPMENT_FOR == 2 ? '('.$row->MERCHANT_NAME. ')' : '(AMT)' }}
                                                    {{ $row->CODE }}
                                                </td>
                                                <td>{{ $row->WAYBILL }}</td>
                                                ...
                                                <td>
                                                    ...
                                                    @endif
                                                    @if($row->SHIPMENT_FOR == 2 && Auth::user()->F_MERCHANT_NO == $row->F_MERCHANT_NO)
                                                        @if(hasAccessAbility('edit_packaging', $roles))
                                                        <a href="{{ route('admin.packaginglist.commarcialpdf',['shipment_no' => $row->PK_NO,'invoice_for' => 'merchant' ]) }}" class="btn btn-xs btn-azura" style="" title="DOWNLOAD COMMERCIAL PDF"><i class="la la-cloud-download"></i> CI</a>
                                                        @endif
                                                    @endif
                                                    @if($row->SHIPMENT_FOR == 2 && Auth::user()->F_MERCHANT_NO == $row->F_MERCHANT_NO)
                                                        @if(hasAccessAbility('edit_packaging', $roles))
                                                        <a href="{{route('admin.packaginglist.pdf',['shipment_no' => $row->PK_NO , 'invoice_for' => 'merchant', 'price' => 'merchant' ]) }}" class="btn btn-xs btn-primary"  title="DOWNLOAD PDF BY USING MERCHANT PRICE"><i class="la la-cloud-download"></i>  P</a>
                                                        @endif
                                                    @endif
