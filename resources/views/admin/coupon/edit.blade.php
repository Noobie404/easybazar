...
@section('page-name') Edit Coupon @endsection
...
<div class="content-body min-height">
    <div class="card card-success">
        <div class="card-body">
            {!! Form::open([ 'route' => ['admin.coupon.update',$row['master']->PK_NO], 'method' => 'post', 'class' => 'form-horizontal','id'=>'coupon_form', 'files' => true ,'novalidate']) !!}
            ...
            <div class="row">
                <div class="col-md-2">
                    <div class="form-group {!! $errors->has('branch_id') ? 'error' : '' !!}">
                        ...
                        <div class="controls">
                            {!! Form::select('branch_id', $data['branch'], $row['master']->F_SHOP_NO, [ 'class' => 'form-control mb-1 select2', 'placeholder' => 'Please select', 'data-validation-required-message' => 'This field is required', 'tabindex' =>1, 'id' => 'branch_id', 'onchange' => "getPurchaser(this)"  ]) !!}
                ...
                <div class="col-md-2">
                    <div class="controls">
                        ...
                        {!! Form::select('coupon_type', array(1=>'Percentage',2=>'Amount'), $row['master']->COUPON_TYPE ?? 1, ['class'=>'form-control mb-1 select2', 'data-validation-required-message' => 'This field is required', 'id' => 'coupon_type', 'required']) !!}
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group {!! $errors->has('discount') ? 'error' : '' !!}">
                        <div class="controls">
                            <label>Enter <span id="coupon_type_span">{{ $row['master']->COUPON_TYPE == 1 ? 'Discount (%)' : 'Amount' }}</span> <span class="text-danger">*</span></label>
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <div class="controls">
                        ...
                        {!! Form::select('coupon_for', array(3=>'Flat discount'), $row['master']->COUPON_ON ?? 1, ['class'=>'form-control mb-1 select2', 'data-validation-required-message' => 'This field is required', 'id' => 'coupon_for', 'required']) !!}
                        {{-- {!! Form::select('coupon_for', array(1=>'Product',2=>'Master',3=>'Flat discount'), $row['master']->COUPON_ON ?? 1, ['class'=>'form-control mb-1 select2', 'data-validation-required-message' => 'This field is required', 'id' => 'coupon_for', 'required']) !!} --}}
                    </div>
                </div>
                {{-- <div class="col-md-4">
                                        ...
                                        <div class="table-responsive fix-table">
                                            <table class="table table-striped table-bordered table-sm" id="selected_list_table">
                                                ...
                                                <tbody id="slected_list_tbody">
                                                    ...
                                                    <tr>
                                                        <td><input id="checkbox" class="ml-1" name="checkbox" type="checkbox" value="{{ $item->F_PRD_VARIANT_NO }}"></td>
                                                        <td><img src="{{ asset('') }}{{ $item->THUMB_PATH }}" style="width : 50px;"></td>
                                                        <td>{{ $item->VARIANT_NAME }}</td>
                                                        <td>
                                                            <a href="{{ route('admin.product.searchlist.view',[$item->F_PRD_MASTER_SETUP_NO,'variant_id'=>$item->PK_NO,'type'=>'variant','tab'=>2]) }}" target="_blank" class="btn btn-xs btn-info mr-1"><i class="la la-eye"></i></a>
                                                            <input id="to_show" class="ml-1" name="to_show[]" type="checkbox" value="{{ $item->F_PRD_VARIANT_NO }}" {{ $item->TO_SHOW == 1 ? 'checked' : '' }}>
                                                        </td>
                                                        <script>
                                                            selected_item_list.push(`{{ $item->F_PRD_VARIANT_NO }}`);
                                                        </script>
                                                    </tr>
                                                    @endforeach
                                                    <script>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group {!! $errors->has('image') ? 'error' : '' !!}">
                        ...
                        <div class="controls">
                            <div class="fileupload @if(!empty($row['master']->COUPON_IMAGE))  {{'fileupload-exists'}} @else {{'fileupload-new'}} @endif " data-provides="fileupload" >
                            <span class="fileupload-preview fileupload-exists thumbnail" style="max-width: 150px; max-height: 120px;">
                            @if(!empty($row['master']->COUPON_IMAGE))
                            <img src="{{asset($row['master']->COUPON_IMAGE)}}" alt="Photo" class="img-fluid" height="150px" width="120px"/>
                            @endif
                            </span>
                            <span>
