...
<head>
    ...
    <style>
        @page{margin:0 0}@font-face{font-family:Helvetica;font-weight:400;font-style:normal;font-variant:normal;src:url("font url")}header{position:fixed;top:1cm;left:1cm;right:1cm;height:10cm}body{margin-top:7cm;margin-left:1cm;margin-right:1cm;margin-bottom:5.1cm;font-family:Helvetica,sans-serif;font-size:11px}footer{position:fixed;bottom:0;left:0;right:0;height:5cm;text-align:center;font-size:11px}.tbl-list{border-top:1px solid #000;border-bottom:0;border-left:1px solid #000;border-right:0;border-spacing:0;border-collapse:separate}.tbl-list td,.tbl-list th{border-bottom:1px solid #000;border-right:1px solid #000;border-top:0;border-left:0}.tbl-list tr td:last-child{border-right:1px solid #000}.tbl-list tr td:first-child{border-left:0}.tbl-list tr:first-child{border-top:0}tfoot tr td{font-weight:700}.page-break{page-break-after:always}.tbl-header{background-color:#0073b7;color:#fff}#tbl-signature{width:70%;margin:10px auto;border-spacing:30px 0;margin-top:10px;margin-bottom:10px}
    </style>
    <?php
    ...
    <body>
        ...
        <footer>
			...
			<table id="tbl-signature" style="margin-bottom: 0px; padding-bottom: 0px;">
				<tr>
...
$grand_total_amt = 0;
            ...
            <table width="100%" class="tbl-list" style="margin-bottom: 30px;" cellpadding="3" cellspacing="0">
                ...
                @if(isset($data['data']) && count($data['data']) > 0 )
                    @foreach($data['data'] as $ik => $item )
                        ...
                        if($data['price'] == 'merhcnat' && $item->MER_UNIT_PRICE != ''){
                            $unit_price = $item->MER_UNIT_PRICE;
                        }

                        $total_box_qnty   += $item->QTY;
                        $total_box_amount += $item->QTY*$unit_price;

                        $grand_total_qty += $item->QTY;
                        $grand_total_amt += $item->QTY*$unit_price;
                        ?>
                ...
                <tr>
                    ...
                    <td width="11%" align="right">{{ number_format($unit_price,4,".","") }}</td>
                    <td width="9%" align="right">{{ number_format(($item->QTY*$unit_price),2,".","") }}</td>
                </tr>
                @endif
                @endforeach
                @endif
        ...
        <table class="tbl-list" border="1" width="50%" style="margin-bottom: 30px; margin:auto;" cellpadding="3" cellspacing="0">
            ...
            <tbody>
                ...
                if($data['price'] == 'merhcnat' && $bs->MER_BOX_TOTAL_PRICE > 0){
                    $box_total_price = $bs->MER_BOX_TOTAL_PRICE;
                }else{
                    $box_total_price = $bs->BOX_TOTAL_PRICE;
                }


                $box_total_price += $box_total_price ;
                ?>
                <tr>
        ...
        <table class="tbl-list" border="1" width="50%" style="margin-bottom: 20px; margin:auto;" cellpadding="3" cellspacing="0">
            ...
            <tbody>
                ...
                <?php
                    ...
                    if($data['price'] == 'merhcnat' && $hs->MER_BOX_TOTAL_PRICE > 0){
                        $box_total_price = $hs->MER_BOX_TOTAL_PRICE;
                    }else{
                        $box_total_price = $hs->BOX_TOTAL_PRICE ;
                    }

                    $hs_total_price 	+= $box_total_price ;
