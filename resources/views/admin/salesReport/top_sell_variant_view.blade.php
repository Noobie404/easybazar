...
@section('title') Top Sell Variant List @endsection
@section('page-name') Top Sell Variant List @endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">@lang('shipping.breadcrumb_title')</a></li>
    <li class="breadcrumb-item active">Top Sell Variant List</li>
@endsection

@php
    $roles = userRolePermissionArray();
    use Carbon\Carbon;
    $master_no = $data['top_sell_master']->PK_NO;
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                <div class="row well add_section" style="background-color: #b5b3b555; padding: 20px;">
                                    <div class="col-12">
                                    {!! Form::open([ 'route' => 'admin.top_sell_variant.create', 'method' => 'post', 'class' => 'form-horizontal top_sell_create', 'files' => true , 'novalidate']) !!}
                                        <input type="hidden" value="{{ $data['top_sell_master']->F_BEST_SELL_MASTER_NO }}" name="top_sell_master">
                                ...
                                <a href="{{ route('admin.top_sell.list') }}" class="text-white btn btn-sm btn-info" id="bulk_delete" title="Delete" >Back</a>
                            </div>
                        </div>
                    </div>
                </div>
...
var get_url = $('#base_url').val();
    ...
    $(document).on("click", "#bulk_delete", function(event){
        ...
        if (draft != '') {
            var url = get_url + '/top-sell-variant/delete';
            if(confirm('Are you sure?')) {
                $.ajax({
                    url: url,
                    type: 'POST',
...
<script>
    ...
    function getProduct(){
        var get_top_sell_view = `{{ route('admin.get_top_sell_variant', $master_no) }}`;
        var table =
    ...
    $('.select2-multiple').select2({
            ajax: {
            url: "{{route('get-topsell-variant', $master_no)}}",
            type: "post",
            dataType: 'json',
            delay: 250,
