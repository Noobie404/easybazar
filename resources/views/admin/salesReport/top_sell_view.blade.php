...
@section('title') Top Sell Product List @endsection
@section('page-name') Top Sell Product List @endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">@lang('shipping.breadcrumb_title')</a></li>
    <li class="breadcrumb-item active">Top Sell Product List</li>
@endsection
@php
                        ...
                        <div class="card-content collapse show">
                            <div class="card-body card-dashboard">
                                <div class="row well add_section" style="background-color: #b5b3b555; padding: 20px;">
                                    <div class="col-12">
                                    {!! Form::open([ 'route' => 'admin.top_sell_master.create', 'method' => 'post', 'class' => 'form-horizontal top_sell_create', 'files' => true , 'novalidate']) !!}
                                        <input type="hidden" value="{{ $data->PK_NO }}" name="top_sell_master">
                                ...
                                <a href="{{ route('admin.top_sell.list') }}" class="text-white btn btn-sm btn-info" title="Delete" >Back</a>
                            </div>
                        </div>
                    </div>
                </div>
...
$(document).on('submit', '.top_sell_create', function(){
  ...
  return false;
    ...
    $(document).on("click", "#bulk_delete", function(event){
        ...
        if (draft != '') {
            var url = get_url + '/top-sell/delete';
            if(confirm('Are you sure?')) {
                $.ajax({
                    url: url,
                    type: 'POST',
...
<script>
    ...
    function getProduct(){
        var get_top_sell_view = `{{ route('admin.get_top_sell_view', $data->PK_NO) }}`;
        var table =
